...
# GeckoLib 1.8.0 uses CatmullRom interpolation by default for ALL channels
# (rotation, position, scale). The previous setting used "linear" for position
# and scale, which caused walk animations to appear jerky — the sparse position
# keyframes with linear interpolation created abrupt transitions that looked like
# the animation "resetting" every ~0.5s. The fix: set ALL channels to catmullrom,
# then the CatmullRom baker produces dense linear keyframes that approximate the
# smooth CatmullRom curve, matching GeckoLib's runtime behavior.
