...
def batch_convert_mdo_srp(
    ...
    for i, rel_path in enumerate(geo_files, 1):
        ...
        try:
            ...
            # causing CatmullRom tangent distortion at the loop boundary. The
            # reference SubspaceParasite converter avoids this by extending short
            # loop animations to multiple cycles (3-4x the source length).
            # This pushes the loop boundary far from the dense keyframe region
            # and makes the CatmullRom wrapping distortion negligible.

            if animations_ir:
                orig_count = len(animations_ir)
                animations_ir = extend_loop_animations(animations_ir, name)
                extended = sum(1 for a in animations_ir if a.length > 3.0)
                if extended > 0:
                    status_parts.append(f"loop_ext({extended})")
