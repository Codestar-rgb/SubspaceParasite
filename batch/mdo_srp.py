...
def batch_convert_mdo_srp(
    ...
    for i, rel_path in enumerate(geo_files, 1):
        ...
        try:
            ...
            # NOTE (v6.1): loop_extender is a NO-OP that preserves original
            # animation_length (see engine/loop_extender.py docstring).
            # CatmullRom baking (step 3e) handles loop boundary distortion.
            # We keep the call for pipeline compatibility but report honestly.
            if animations_ir:
                animations_ir = extend_loop_animations(animations_ir, name)
                # Report animations with length > 3.0s as "long" (informational
                # only — they were NOT extended; original lengths are preserved).
                long_count = sum(1 for a in animations_ir if a.length > 3.0)
                if long_count > 0:
                    status_parts.append(f"long_anim({long_count})")
