...
def enforce_loop_continuity(sampled_data: Dict, duration: float, blend_ratio: float = 0.15) -> Dict:
    ...
    Since animations use incommensurate frequencies that can never perfectly
    return to start simultaneously, we FORCE loop closure by:
    
    1. Creating a smooth blend zone in the last `blend_ratio` portion of the
       animation where values gradually transition from their natural trajectory
       to the starting values using a cosine ease function.
    2. Ensuring the last keyframe EXACTLY matches the first.
    
    The blend uses cosine interpolation for C1-continuous (velocity smooth) transitions:
      blend_weight = 0.5 * (1 - cos(pi * progress))
    where progress goes from 0 to 1 over the blend zone.
    """
    blend_start = duration * (1.0 - blend_ratio)
    ...
    for bone_name, channels in sampled_data.items():
        for channel, keyframes in channels.items():
            ...
            # If already matching (within floating point), just snap
            if abs(first_val - last_val) < 1e-10:
                keyframes[-1] = (keyframes[-1][0], first_val)
                continue
            
            # Apply smooth blending in the last portion of the animation
            # For each keyframe in the blend zone, interpolate toward the start value
            for i in range(len(keyframes)):
                t, val = keyframes[i]
                
                if t >= blend_start and t < duration:
                    # Calculate blend progress (0 at blend_start, 1 at duration)
                    progress = (t - blend_start) / (duration - blend_start)
                    # Cosine ease for smooth C1-continuous transition
                    blend_weight = 0.5 * (1.0 - math.cos(math.pi * progress))
                    # Blend from natural value toward start value
                    blended_val = val * (1.0 - blend_weight) + first_val * blend_weight
                    keyframes[i] = (t, blended_val)
            
            # Force last keyframe to exactly match first
            keyframes[-1] = (keyframes[-1][0], first_val)
...
def generate_all_animations() -> dict:
    ...
    Quality v3 improvements:
    - Higher sampling rates (90-480 fps) for precision and smoothness
    - Tighter DP epsilon (0.03-0.06) for preserving subtle motion detail
    - All 8 animation states: idle, attack, fly, vomit, fly_vomit, shaking, cosmic, cosmic_shaking
    - Loop-aligned durations for seamless looping
    - Enforced loop continuity on all loop animations
    """
    animations = {}
    ...
    idle_data = sample_animation(
        ...
        samples_per_second=90.0,  # Higher rate for smoother motion
        dp_epsilon=0.03  # Tighter for more keyframe precision
    )
    ...
    attack_data = sample_animation(
        ...
        samples_per_second=90.0,  # Higher rate
        dp_epsilon=0.03  # Tighter for attack detail
    )
    ...
    fly_data = sample_animation(
        ...
        samples_per_second=90.0,  # Higher rate for wing flap detail
        dp_epsilon=0.03  # Tight epsilon for wing flap precision
    )
    ...
    # Vomit neck cycle: cos(ageInTicks * 0.061) → freq = 0.061*20 = 1.22 rad/s
    # Period = 2π/1.22 ≈ 5.15s → Use 1 full cycle for looping
    vomit_neck_period = 2 * math.pi / (0.061 * TICKS_PER_SECOND)
    vomit_duration = vomit_neck_period  # ~5.15s for one full neck cycle
    
    vomit_data = sample_animation(
        lambda t: eval_vomit(t, raining=False, flying=False),
        duration=vomit_duration,
        samples_per_second=90.0,  # Higher rate for neck articulation detail
        dp_epsilon=0.02  # Very tight epsilon for neck detail
    )
    vomit_data = enforce_loop_continuity(vomit_data, vomit_duration)
    ...
    # Fly + vomit: same wing period as fly, with vomit neck
    fly_vomit_duration = fly_duration  # Same as fly for wing cycle alignment
    
    fly_vomit_data = sample_animation(
        lambda t: eval_vomit(t, raining=False, flying=True),
        duration=fly_vomit_duration,
        samples_per_second=90.0,
        dp_epsilon=0.03
    )
    fly_vomit_data = enforce_loop_continuity(fly_vomit_data, fly_vomit_duration)
    ...
    # Duration: use period that allows both walk cycle and shake to align
    # Walk cycle: 2.327s, shake cycle: 0.1066s
    # Use walk cycle period since shaking includes walking
    shaking_duration = idle_walk_period  # Align with walk cycle for smooth loop
    
    shaking_data = sample_animation(
        eval_shaking,
        duration=shaking_duration,
        samples_per_second=960.0,  # Very high rate for fast oscillation sampling
        dp_epsilon=0.02  # Tight epsilon to preserve subtle 0.089 amplitude
    )
    ...
    # Use walk cycle period for alignment
    cosmic_duration = idle_walk_period  # Align with walk cycle for smooth loop
    
    cosmic_data = sample_animation(
        lambda t: eval_cosmic(t, shaking=False),
        duration=cosmic_duration,
        samples_per_second=480.0,  # Higher rate for fast oscillation
        dp_epsilon=0.03  # Tighter for cosmic shake detail
    )
    ...
    # This is VERY fast - needs 480+ fps for adequate sampling
    cosmic_shaking_duration = idle_walk_period  # Align with walk cycle for smooth loop
    
    cosmic_shaking_data = sample_animation(
        lambda t: eval_cosmic(t, shaking=True),
        duration=cosmic_shaking_duration,
        samples_per_second=960.0,  # Very high rate for extremely fast oscillation
        dp_epsilon=0.02  # Tight epsilon to preserve dampened 0.59*0.3=0.177 amplitude
    )
