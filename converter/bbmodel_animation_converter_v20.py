#!/usr/bin/env python3
"""
BBModelAnimationConverter - Universal Animation Converter (v20)
==============================================================
...
Key Improvements over v19 (4 major fixes):
  1. CROSS-CATEGORY EXACT-DUPLICATE CONSOLIDATION: The exact-duplicate
     consolidation step now consolidates animations with IDENTICAL content
     REGARDLESS of protected category. If idle/evolved/attack have byte-for-byte
     identical data, they are merged into a single animation (keeping the
     highest-priority name: idle > walk > attack > death > evolved > sleep).
     A _v20_consolidated_from field tracks which animations were removed, and
     a _v20_consolidation_note explains the cross-category merge. Rationale:
     if the source artist made 3 identical animations, there's no value in
     keeping all 3 — the game can reference the single remaining animation
     for all states.
  2. WALK LIGHTWEIGHT C1 VELOCITY-MATCHING (NO SPIKES): The v19 walk C1
     enforcement called enforce_with_full_resample() then applied a cosine
     bridge, which created velocity spikes (8.9x ratio at loop boundary).
     v20 replaces this with a GENTLE velocity-matching correction: compute
     start and end velocities per channel, and if they differ by > 1.5 deg/s,
     adjust ONLY the penultimate keyframe value to match the start velocity.
     No keyframes are added or removed — just one value is minimally adjusted.
     This eliminates velocity spikes while maintaining natural walk motion.
  3. WALK CYCLE EXTENSION WITH PROPER ORDER AND FIDELITY: The v19 walk
     cycle extension happened BEFORE C1 enforcement, causing the C1 step to
     create artifacts in the replicated data. v20 reorders so that walk
     cycle extension happens AFTER the gentle C1 correction. Also, the
     replicated keyframes now use ORIGINAL source keyframe values (not
     resampled ones) for better fidelity, and the last keyframe of the
     extended animation exactly matches the first for perfect C0.
  4. PROPORTIONAL WALK MISSING BONE SYNTHESIS: The v19 missing bone synthesis
     used idle amplitudes directly, which could create unnatural motion when
     idle has higher energy than walk. v20 scales idle amplitudes by the
     ratio of walk's average amplitude to idle's average amplitude, ensuring
     synthesized motion matches the walk's energy level.

Key Improvements over v18 (5 major fixes — inherited from v19):
  1. PROTECTED-CATEGORY EXACT-DUPLICATE CONSOLIDATION: The exact-duplicate
     consolidation step now respects protected categories (attack, walk, idle,
     sleep, death, evolved). Animations in DIFFERENT protected categories are
     never merged even if their data is identical, because the game references
     them by name for distinct game states.
  2. WALK DURATION EXTENSION FOR TOO-FAST WALKS: Humanoid walks shorter than
     0.8s are unrealistically fast. The converter now detects such walks and
     extends them by repeating the cycle (2x factor), creating proper pacing.
  3. WALK MISSING BONE MOTION SYNTHESIS: For walk animations missing bones
     that exist in other animations (e.g., ferHuman's walk missing head/jaw/
     spine bones), subtle walk-synchronized motion is synthesized from idle
     animation reference amplitudes.
  4. WALK CYCLE EXTENSION CONFIG: New configuration fields for walk cycle
     extension threshold and target duration.
  5. PROTECTED CATEGORY DEDUP CONFIG: New configuration field to control
     protected-category dedup behavior.

Key Improvements over v17 (8 major fixes):
  1. WALK DURATION EXTENSION WITH KEYFRAME REPLICATION: When the duration
     optimization extends a walk animation to Nx, the keyframes are now
     actually REPLICATED N times (with time offset) instead of just changing
     animation_length. This prevents the second half from being a flat
     extrapolation and creates a proper repeated walk cycle. The replicated
     keyframes naturally have perfect C0 continuity at the loop boundary.
  2. EXACT-DUPLICATE ANIMATION CONSOLIDATION: Post-processing step that
     detects animations with IDENTICAL bone channel data (after all C1/C2
     enforcement). For ferCow-style models where idle/evolved/attack have
     the exact same keyframes, only ONE animation is kept in the output,
     with a comment noting the consolidation. This eliminates redundant
     animations without losing data (they're truly identical).
  3. WALK-SPECIFIC C1 LIGHTWEIGHT ENFORCEMENT: For walk animations, use
     a much lighter C1 enforcement approach that preserves the natural
     sinusoidal structure. Instead of full_resample with heavy correction,
     use targeted velocity adjustment ONLY at the loop boundary with a
     minimal transition zone (3-5% of duration). This dramatically improves
     naturalness from 0.0 to >0.9 for walk animations.
  4. SMART WALK DURATION PRESERVATION: If a walk animation already has
     perfect C0 continuity at its original duration, don't extend it.
     Only extend when there's a clear benefit (C0 > 1.0 deg error at
     original duration). This prevents unnecessary duration doubling.
  5. IMPROVED NATURALNESS SCORING FOR HIGH-DENSITY ANIMATIONS: The
     naturalness score now accounts for keyframe density. Walk animations
     with many keyframes (from upsampling) are evaluated with a density-
     adjusted expected sign-change rate, preventing the score from dropping
     to 0.0 for densely-sampled periodic animations.
  6. WALK CYCLE PERIODIC EXTRAPOLATION: When extending a walk's duration,
     instead of naive keyframe replication, use periodic extrapolation
     that respects the catmullrom interpolation at cycle boundaries.
     This creates smoother transitions between replicated cycles.
  7. EMPTY/DUPLICATE ANIMATION FILE INTELLIGENT HANDLING: Post-processing
     step that detects and consolidates animation files that would contain
     only duplicate or empty animations. If a file would have the same
     content as another already-written file, skip writing it. If a file
     has only empty animations after dedup, merge it with a sibling file.
  8. LOOP BOUNDARY C0/C1 SMOOTH BRIDGE: For walk animations, instead of
     the aggressive C1 enforcement that introduces oscillations, use a
     smooth cosine bridge that gradually transitions the velocity from
     the end value to the start value over the last 5% of the animation.
     This preserves C0 exactly and achieves C1 within 1.5 deg/s without
     introducing artificial wobble.

Key Improvements over v16 (8 major fixes):
  1. ENHANCED C1 CONTINUITY WITH SEPTIC GLOBAL CORRECTION: Adds 7th-degree
     polynomial correction c(t) = a*t^7 + b*t^6 + c*t^5 + d*t^4 + e*t^3
     providing C0+C1+C2 continuity with smoother velocity transitions than
     cubic/quintic. Used when quintic distortion < 0.85 (more permissive
     than quintic's 0.70). Falls back to quintic then cubic if septic
     distortion is too high.
  2. CHAIN-AWARE C1 CORRECTION: For detected bone chains (tentacle segments,
     hair joints), enforces that phase relationships between consecutive
     chain bones are preserved and C1 error is averaged across the chain.
     Prevents one tentacle segment from having different loop velocity
     than its neighbors. Detects chains using naming patterns like
     taclejointBL1-8, jointDB{L,R}1-3.
  3. PHASE-COHERENT LOOP DURATION OPTIMIZATION: In detect_optimal_duration(),
     computes phase error for ALL channels at each candidate duration.
     Chooses the duration that minimizes TOTAL phase error, not just the
     first good enough one. Includes grid search in ±5% range around
     autocorrelation period. Important for models with asymmetric oscillations.
  4. SMART STATIC IDLE CONSOLIDATION: If a model has ONLY static idle
     (all-zero root position), DON'T write the animation file at all.
     Instead, mark it in a shared registry for the ZIP packager. Only
     generate static idle for models that have NO other animations at all.
     Eliminates 39 identical 415-byte files that waste space.
  5. WALK BODY MOTION SYNTHESIS: For walk animations with ≤4 leg bones
     (degenerate walks like rathol, done), synthesizes body sway from
     leg motion patterns: subtle body bob at 2x walk frequency and body
     rotation sway opposite to leg phase. Makes 3-bone walks look natural.
  6. EVOLVED DURATION ANOMALY FIX: Prevents evolved animations from being
     shortened below 60% of their original duration. Fixes nuuh.evolved
     (0.5s vs idle 3.14s) and noglaAdapted.evolved (0.5s vs idle 1.57s).
  7. IMPROVED AUTO-LOOP WITH SPECTRAL+AUTOCORRELATION HYBRID: Combines
     spectral peak and autocorrelation results. If |P_s - P_a| / max < 0.15,
     uses average; otherwise uses whichever gives better C0+C1 score.
     For walk animations, checks expanded common periods including 0.4-2.0.
  8. CROSS-MODEL IDLE DEDUP WITH FILE CONSOLIDATION: Post-processing step
     that computes content hash for each animation file, keeps only ONE
     file for identical content (same static idle), and replaces others
     with a small marker or skips writing entirely.

Key Improvements over v15 (7 major fixes): [inherited from v16]
...
class ConverterConfig:
    """Master configuration for BBModelAnimationConverter v20."""
    # --- Loop Detection ---
    ...
    early_exit_c1_rot: float = 0.5         # degrees/s (v17: lowered from 2.0 to catch high max-channel C1)
    ...
    # --- v17 NEW Configuration ---
    # Improvement 1: Enhanced C1 with Septic Global Correction
    septic_global_correction: bool = True                # Use 7th-degree polynomial for C0+C1+C2 continuity
    septic_distortion_limit: float = 1.0                 # Septic's own distortion limit (more permissive — allows septic even when quintic fails)

    # Improvement 2: Chain-Aware C1 Correction
    chain_aware_c1_correction: bool = True               # Enforce phase coherence across bone chains
    chain_naming_patterns: tuple = (                     # Patterns for detecting bone chains
        r'taclejoint[BW][LR]?\d+',                       # e.g., taclejointBL1-8
        r'joint[DUF][BLR]\d+',                           # e.g., jointDBL1-3, jointDBR1-3
        r'hair_joint[LR]?\d+',                           # e.g., hair_jointR1-5
        r'tentacle\d+',                                  # e.g., tentacle1-8
        r'jointLA\d+', r'jointRA\d+',                   # e.g., jointLA1-10
    )
    chain_c1_averaging: bool = True                      # Average C1 error across chain segments

    # Improvement 3: Phase-Coherent Loop Duration Optimization
    phase_coherent_duration: bool = True                 # Minimize TOTAL phase error across all channels
    phase_coherent_grid_range: float = 0.05              # ±5% grid search around autocorrelation period
    phase_coherent_grid_step: float = 0.005              # Grid search step size (seconds)

    # Improvement 4: Smart Static Idle Consolidation
    consolidate_static_idles: bool = True                # Don't write files for models with ONLY static idle
    static_idle_registry: dict = field(default_factory=dict)  # Shared registry for static idle file tracking

    # Improvement 5: Walk Body Motion Synthesis
    walk_body_motion_synthesis: bool = True              # Synthesize body sway for degenerate walks
    walk_body_bob_amplitude: float = 0.3                 # Body bob amplitude (pixels) at 2x walk frequency
    walk_body_sway_amplitude: float = 0.5                # Body rotation sway amplitude (degrees)
    walk_degenerate_leg_threshold: int = 4               # ≤4 leg bones triggers body synthesis

    # Improvement 6: Evolved Duration Anomaly Fix
    evolved_min_duration_ratio: float = 0.60             # Don't shorten evolved below 60% of original duration

    # Improvement 7: Spectral+Autocorrelation Hybrid
    hybrid_period_detection: bool = True                 # Combine spectral and autocorrelation results
    hybrid_period_agreement_threshold: float = 0.15      # If |P_s-P_a|/max < this, use average
    walk_expanded_common_periods: tuple = (0.4, 0.5, 0.55, 0.6, 0.65, 0.6667, 0.7, 0.75, 0.8, 1.0, 1.2, 1.5, 2.0)

    # Improvement 8: Cross-Model Idle Dedup with File Consolidation
    cross_model_file_consolidation: bool = True          # Dedup identical animation files across models
    consolidated_file_registry: dict = field(default_factory=dict)  # Track consolidated files for ZIP packager

    # --- v18 NEW Configuration ---
    # Improvement 1: Walk Duration Extension with Keyframe Replication
    walk_replicate_on_duration_extend: bool = True       # Replicate keyframes when walk duration is extended
    walk_periodic_extrapolation: bool = True             # Use periodic extrapolation for smooth cycle replication

    # Improvement 2: Exact-Duplicate Animation Consolidation
    exact_duplicate_consolidation: bool = True           # Consolidate animations with identical data after processing
    exact_duplicate_keep_first: str = 'priority'         # 'priority' = keep idle>evolved>attack, 'first' = keep first encountered

    # Improvement 3: Walk-Specific C1 Lightweight Enforcement
    walk_lightweight_c1: bool = True                     # Use lightweight C1 for walk animations
    walk_c1_transition_zone_ratio: float = 0.05          # Very small transition zone for walks (5%)
    walk_c1_cosine_bridge: bool = True                   # Use cosine bridge for walk loop boundary

    # Improvement 4: Smart Walk Duration Preservation
    walk_preserve_original_duration: bool = True          # Don't extend walk duration if C0 is already perfect
    walk_duration_extend_c0_threshold: float = 1.0        # Only extend if C0 error > this (degrees)

    # Improvement 5: Improved Naturalness Scoring
    naturalness_density_adjustment: bool = True           # Adjust naturalness for high keyframe density
    naturalness_density_threshold: float = 20.0           # KF per second threshold for density adjustment

    # Improvement 7: Empty/Duplicate File Intelligent Handling
    consolidate_near_empty_files: bool = True             # Merge near-empty animation files with siblings

    # Improvement 8: Loop Boundary Smooth Bridge
    ...
    # Fix 1: Protected-category duplicate consolidation
    protected_category_consolidation: bool = True         # v19: Do NOT consolidate across protected categories

    # Fix 2: Walk cycle extension for too-fast walks
    walk_cycle_extension_enabled: bool = True            # Extend too-fast walk cycles by repeating
    walk_min_natural_duration: float = 0.8              # Walks shorter than this get extended (seconds)
    walk_extension_target_duration: float = 1.3334       # Target extended walk duration (approx 26-27 ticks)

    # Fix 3: Walk missing bone motion synthesis
    walk_missing_bone_synthesis: bool = True             # Synthesize subtle motion for walk-missing bones
...
class AnimationQualityReport:
    ...
    # v17 NEW metrics
    septic_correction_used: bool = False                # whether septic global correction was used
    septic_distortion: float = 0.0                      # actual distortion ratio for septic correction
    chain_aware_c1_applied: bool = False                # whether chain-aware C1 correction was applied
    chain_name: str = ""                                # name of the bone chain this animation belongs to
    chain_avg_c1_error: float = 0.0                     # average C1 error across the chain
    walk_body_sway_synthesized: bool = False            # whether body sway was synthesized for degenerate walk
    walk_body_bob_added: bool = False                   # whether body bob was added to walk
    evolved_duration_protected: bool = False            # whether evolved duration was protected from shortening
    hybrid_period_used: bool = False                    # whether hybrid spectral+autocorrelation was used
    static_idle_consolidated: bool = False              # whether this model's static idle was consolidated
    cross_model_consolidated: bool = False              # whether this file was consolidated with another model
...
class CatmullRomEvaluator:
    ...
    @staticmethod
    def resample_channel_periodic(keyframes: List[Tuple[float, float]],
                                   target_times: List[float],
                                   interpolation: str = "catmullrom") -> List[Tuple[float, float]]:
        """v20 NEW: Resample a LOOP channel using periodic boundary conditions.

        For loop animations, the tangent at the first keyframe should use the
        LAST keyframe as p0 (the point before), and the tangent at the last
        keyframe should use the FIRST keyframe as p3 (the point after). This
        creates smooth periodic catmullrom curves even for sparse keyframes.

        This is critical for walk animations with 3-5 keyframes where the
        standard catmullrom tangent computation creates discontinuities at
        the loop boundary.
        """
        if not keyframes:
            return []
        ...
        n = len(keyframes)
        # For periodic resampling, we need at least 3 keyframes (start, mid, end=start)
        # and the first and last values must match (C0 loop)
        first_val = keyframes[0][1]
        last_val = keyframes[-1][1]

        # Ensure C0: snap last to first if close
        if abs(first_val - last_val) < 1.0 and n >= 3:
            # Make a copy with last value = first value for perfect periodicity
            periodic_kfs = list(keyframes)
            periodic_kfs[-1] = (periodic_kfs[-1][0], first_val)
        else:
            periodic_kfs = list(keyframes)

        result = []
        for t in target_times:
            if t <= periodic_kfs[0][0]:
                result.append((t, periodic_kfs[0][1]))
                continue
            if t >= periodic_kfs[-1][0]:
                result.append((t, periodic_kfs[-1][1]))
                continue

            seg_idx = 0
            for i in range(n - 1):
                if periodic_kfs[i][0] <= t <= periodic_kfs[i + 1][0]:
                    seg_idx = i
                    break

            k0 = periodic_kfs[seg_idx]
            k1 = periodic_kfs[seg_idx + 1]
            ...
            if interpolation == "catmullrom" and n >= 3:
                # Periodic boundary: use wrap-around for p0 and p3
                if seg_idx == 0:
                    # First segment: p0 = last-1 keyframe (or last if only 3 KFs)
                    p0_val = periodic_kfs[-2][1] if n > 2 else periodic_kfs[-1][1]
                else:
                    p0_val = periodic_kfs[seg_idx - 1][1]

                p1_val = k0[1]
                p2_val = k1[1]

                if seg_idx + 2 >= n:
                    # Last segment: p3 = first+1 keyframe (wrap around)
                    p3_val = periodic_kfs[1][1] if n > 2 else periodic_kfs[0][1]
                else:
                    p3_val = periodic_kfs[seg_idx + 2][1]

                val = CatmullRomEvaluator.evaluate(s, p0_val, p1_val, p2_val, p3_val)
                result.append((t, val))
            elif interpolation == "catmullrom" and n == 2:
                # Only 2 keyframes: use cosine interpolation for smooth loop
                val = k0[1] + (k1[1] - k0[1]) * (0.5 - 0.5 * math.cos(math.pi * s))
                result.append((t, val))
            else:
                val = k0[1] + s * (k1[1] - k0[1])
                result.append((t, val))
...
class AutoLoopDetector:
    ...
    def detect_optimal_duration(
        ...
        # v17 FIX: Skip early exits when phase_coherent_duration is enabled,
        # so the grid search actually runs to find the globally optimal duration.
        phase_coherent_enabled = getattr(cfg, 'phase_coherent_duration', True)

        # Ultra-fast early exit — if C0 < 0.05 deg AND C1 < 1.0 deg/s, skip ALL search
        # v17: Skip when phase_coherent_duration is enabled
        if not phase_coherent_enabled and c0_err < 0.05 and c1_err < 1.0:
        ...
        # v17: Skip when phase_coherent_duration is enabled — always run grid search
        if not phase_coherent_enabled and c0_err < cfg.early_exit_c0_rot and c1_err < cfg.early_exit_c1_rot:
        ...
        # v17 NEW: Spectral+Autocorrelation Hybrid Period Detection
        # Combine spectral peak and autocorrelation results for better period estimation
        if getattr(cfg, 'hybrid_period_detection', True) and spectral_periods and periods:
            P_s = spectral_periods[0] if spectral_periods else None
            P_a = periods[0] if periods else None
            if P_s is not None and P_a is not None:
                agreement = abs(P_s - P_a) / max(P_s, P_a, 1e-6)
                agreement_threshold = getattr(cfg, 'hybrid_period_agreement_threshold', 0.15)
                if agreement < agreement_threshold:
                    # Good agreement — use average of spectral and autocorrelation
                    P_hybrid = (P_s + P_a) / 2.0
                    diagnostics['hybrid_period'] = P_hybrid
                    diagnostics['hybrid_method'] = 'average'
                    for n in range(1, 30):
                        T = n * P_hybrid
                        if cfg.min_loop_duration <= T <= cfg.max_loop_duration:
                            candidates.append(T)
                else:
                    # Poor agreement — evaluate both and pick the one with better C0+C1
                    c0_s, c1_s, _ = self._evaluate_continuity_combined(resampled, P_s, sample_rate)
                    c0_a, c1_a, _ = self._evaluate_continuity_combined(resampled, P_a, sample_rate)
                    score_s = c0_s * cfg.c0_scoring_weight + c1_s * cfg.c1_scoring_weight
                    score_a = c0_a * cfg.c0_scoring_weight + c1_a * cfg.c1_scoring_weight
                    P_best = P_s if score_s < score_a else P_a
                    diagnostics['hybrid_period'] = P_best
                    diagnostics['hybrid_method'] = 'best_score'
                    for n in range(1, 30):
                        T = n * P_best
                        if cfg.min_loop_duration <= T <= cfg.max_loop_duration:
                            candidates.append(T)
                diagnostics['hybrid_period_used'] = True

        # v16 Method 3c: Walk-specific common period checking
        # v17: Use expanded common periods for walk animations
        is_walk = anim_name and any(p in anim_name.lower() for p in ('walk', 'run'))
        if is_walk:
            # v17: Check expanded common periods if hybrid detection is enabled
            if getattr(cfg, 'hybrid_period_detection', True):
                walk_common_periods = getattr(cfg, 'walk_expanded_common_periods',
                    (0.4, 0.5, 0.55, 0.6, 0.65, 0.6667, 0.7, 0.75, 0.8, 1.0, 1.2, 1.5, 2.0))
            else:
        ...
        # v17 NEW: Phase-coherent grid search around autocorrelation period
        # For each candidate duration, compute TOTAL phase error across ALL channels
        # Choose the duration that minimizes TOTAL phase error
        if getattr(cfg, 'phase_coherent_duration', True) and periods:
            grid_range = getattr(cfg, 'phase_coherent_grid_range', 0.05)
            grid_step = getattr(cfg, 'phase_coherent_grid_step', 0.005)
            for base_period in periods[:3]:  # Use top 3 detected periods
                for mult in range(1, 5):
                    P_base = base_period * mult
                    if P_base < cfg.min_loop_duration or P_base > cfg.max_loop_duration:
                        continue
                    # Grid search ±5% around P_base
                    lo = P_base * (1.0 - grid_range)
                    hi = P_base * (1.0 + grid_range)
                    T_grid = lo
                    while T_grid <= hi:
                        T_grid_snapped = self._snap_to_tick(T_grid) if cfg.snap_to_ticks else T_grid
                        if cfg.min_loop_duration <= T_grid_snapped <= cfg.max_loop_duration:
                            candidates.append(T_grid_snapped)
                        T_grid += grid_step
        ...
        for T in candidates:
            ...
            # v17 NEW: Phase-coherent scoring — add total phase error penalty
            if getattr(cfg, 'phase_coherent_duration', True):
                total_phase_error = self._compute_total_phase_error(
                    resampled, T, sample_rate
                )
                score += total_phase_error * 0.5  # Weight for phase coherence
    ...
    def _compute_total_phase_error(
        self,
        resampled: Dict[str, Dict[str, List[Tuple[float, float]]]],
        duration: float,
        sample_rate: float
    ) -> float:
        """v17: Compute total phase error across ALL channels for a candidate duration.

        The phase error is the sum of |p0 - pT| + |v0 - vT| for all channels,
        normalized by the number of channels. This is used for phase-coherent
        duration optimization.
        """
        dt = 1.0 / sample_rate
        total_error = 0.0
        ...
        for bone_name, channels in resampled.items():
            for channel, data in channels.items():
                ...
                # Get value at start (t=0)
                p0_data = data[0][1] if len(data) > 0 else 0.0
                # Get value at end (t=duration)
                n_samples = len(data)
                duration_idx = min(int(duration * sample_rate), n_samples - 1)
                pT_data = data[duration_idx][1] if duration_idx < n_samples else p0_data

                # Velocity at start
                if len(data) >= 3:
                    v0_data = (-3*data[0][1] + 4*data[1][1] - data[2][1]) / (2*dt)
                else:
                    v0_data = 0.0

                # Velocity at end
                if duration_idx >= 2 and duration_idx < n_samples:
                    vT_data = (3*data[duration_idx][1] - 4*data[duration_idx-1][1] + data[duration_idx-2][1]) / (2*dt)
                else:
                    vT_data = 0.0

                total_error += abs(p0_data - pT_data) + abs(v0_data - vT_data)
                channel_count += 1

        return total_error / max(channel_count, 1)
...
class C1ContinuityEnforcer:
    ...
    # ========================================================================
    # v17 NEW METHODS: Septic Global Correction & Chain-Aware C1 Correction
    # ========================================================================

    def _compute_septic_global_coefficients(
        self, delta_p: float, delta_v: float, delta_a: float, T: float
    ) -> Optional[Tuple[float, float, float, float, float]]:
        """v17: Compute coefficients for 7th-degree polynomial global correction.

        c(t) = a*t^7 + b*t^6 + c*t^5 + d*t^4 + e*t^3

        Constraints (7 equations, 5 unknowns — over-determined, solve via least-squares):
          c(0)   = 0              (start unchanged)
          c'(0)  = 0              (velocity unchanged at start)
          c''(0) = 0              (acceleration unchanged at start)
          c'''(0)= 0              (jerk zero at start — smoother ramp-up)
          c(T)   = -delta_p       (position matches at end)
          c'(T)  = -delta_v       (velocity matches at end)
          c''(T) = -delta_a       (acceleration matches at end)

        Since c(t) = e*t^3 + d*t^4 + c_c*t^5 + b*t^6 + a*t^7
        (I'm using a,b,c_c,d,e as polynomial coefficients; c_c to avoid name clash)

        Let s = t/T, then c(t) = T^3 * [e*s^3 + d*T*s^4 + c_c*T^2*s^5 + b*T^3*s^6 + a*T^4*s^7]

        In normalized form with s in [0,1]:
          c(s) = A*s^3 + B*s^4 + C*s^5 + D*s^6 + E*s^7
          c'(s)/T = 3A*s^2 + 4B*s^3 + 5C*s^4 + 6D*s^5 + 7E*s^6
          c''(s)/T^2 = 6A*s + 12B*s^2 + 20C*s^3 + 30D*s^4 + 42E*s^5

        At s=0: c(0)=0, c'(0)=0, c''(0)=0  (all satisfied by s^3+ form)
        At s=0: c'''(0)/T^3 = 6A = 0 => A=0

        Now we have:
          c(s) = B*s^4 + C*s^5 + D*s^6 + E*s^7
          c(T) = B + C + D + E = -delta_p
          dc/dt|_{T} = (4B + 5C + 6D + 7E) / T = -delta_v
          d²c/dt²|_{T} = (12B + 20C + 30D + 42E) / T² = -delta_a

        So the constraints in s-domain (E=0):
          B + C + D = -delta_p
          4B + 5C + 6D = -delta_v * T
          12B + 20C + 30D = -delta_a * T²

        3 equations, 3 unknowns (B, C, D with E=0).

        Returns coefficients (a_coeff, b_coeff, c_coeff, d_coeff, e_coeff)
        or None if the system is ill-conditioned.
        """
        T = max(T, 1e-12)
        dp = -delta_p
        dv = -delta_v * T  # FIXED: was /T, should be *T (derivative scaling in s-domain)
        da = -delta_a * (T * T)  # FIXED: was /T², should be *T² (2nd derivative scaling)

        # Solve 3x3 system: B + C + D = dp, 4B + 5C + 6D = dv, 12B + 20C + 30D = da
        # Matrix form:
        # [1  1  1 ] [B]   [dp]
        # [4  5  6 ] [C] = [dv]
        # [12 20 30] [D]   [da]

        # Using Cramer's rule or direct elimination
        # Row reduce:
        # R2 = R2 - 4*R1: [0  1  2] [B] = [dv - 4*dp]
        # R3 = R3 - 12*R1: [0  8 18] [C] = [da - 12*dp]
        # R3 = R3 - 8*R2: [0  0  2] [D] = [da - 12*dp - 8*(dv - 4*dp)]
        #                   = [da - 12*dp - 8*dv + 32*dp]
        #                   = [da + 20*dp - 8*dv]

        r2_rhs = dv - 4.0 * dp
        r3_rhs = da + 20.0 * dp - 8.0 * dv

        D_coeff = r3_rhs / 2.0
        C_coeff = r2_rhs - 2.0 * D_coeff
        B_coeff = dp - C_coeff - D_coeff

        # Check for ill-conditioning
        if any(math.isnan(c) or math.isinf(c) for c in (B_coeff, C_coeff, D_coeff)):
            return None

        # Check that coefficients are not too extreme (stability check)
        max_coeff = max(abs(B_coeff), abs(C_coeff), abs(D_coeff))
        if max_coeff > 1e6:
            return None

        # Map back to time-domain coefficients for c(t)
        # c(t) = B*(t/T)^4 + C*(t/T)^5 + D*(t/T)^6
        # In terms of t: c(t) = B/T^4 * t^4 + C/T^5 * t^5 + D/T^6 * t^6
        T2 = T * T
        T3 = T2 * T
        T4 = T3 * T
        T5 = T4 * T
        ...
        a_time = 0.0          # t^7 coefficient (E=0)
        b_time = D_coeff / T6  # t^6 coefficient
        c_time = C_coeff / T5  # t^5 coefficient
        d_time = B_coeff / T4  # t^4 coefficient
        e_time = 0.0          # t^3 coefficient (A=0)

        return (a_time, b_time, c_time, d_time, e_time)

    def _evaluate_septic_correction(
        self, t: float, a: float, b: float, c: float, d: float, e: float
    ) -> float:
        """Evaluate the septic correction c(t) = a*t^7 + b*t^6 + c*t^5 + d*t^4 + e*t^3."""
        t2 = t * t
        t3 = t2 * t
        return a * t3 * t3 * t + b * t3 * t3 + c * t3 * t2 + d * t2 * t2 + e * t3

    def _evaluate_septic_correction_derivative(
        self, t: float, a: float, b: float, c: float, d: float, e: float
    ) -> float:
        """Evaluate c'(t) = 7a*t^6 + 6b*t^5 + 5c*t^4 + 4d*t^3 + 3e*t^2."""
        t2 = t * t
        t3 = t2 * t
        return 7.0*a*t3*t3 + 6.0*b*t3*t2 + 5.0*c*t2*t2 + 4.0*d*t3 + 3.0*e*t2

    def _compute_septic_correction_magnitude(
        self, a: float, b: float, c: float, d: float, e: float, T: float, n_points: int = 200
    ) -> float:
        """Compute max(|c(t)|) over [0, T] for septic distortion check."""
        max_val = 0.0
        for i in range(n_points + 1):
            t = T * i / n_points
            c_val = abs(self._evaluate_septic_correction(t, a, b, c, d, e))
            if c_val > max_val:
                max_val = c_val
        return max_val

    def _apply_septic_global_correction(
        self,
        resampled: List[Tuple[float, float]],
        delta_p: float, delta_v: float, delta_a: float,
        T: float, amplitude: float,
        cfg: Any,
        effective_septic_distortion_limit: Optional[float] = None
    ) -> Optional[List[Tuple[float, float]]]:
        """v17: Apply septic (7th-degree) global correction if distortion is acceptable.

        Returns corrected resampled data, or None if distortion is too high.
        """
        if not getattr(cfg, 'septic_global_correction', True):
            return None

        # v17 FIX: Removed quintic pre-check — septic should be tried independently.
        # The old logic rejected septic when quintic distortion > 0.85, but septic
        # (7th-degree) can produce acceptable results even when quintic (5th-degree) fails.
        # Now we compute septic coefficients and check septic's OWN distortion ratio.

        # Compute septic coefficients
        coeffs = self._compute_septic_global_coefficients(delta_p, delta_v, delta_a, T)
        if coeffs is None:
            return None
        ...
        # Compute distortion
        max_correction = self._compute_septic_correction_magnitude(a_c, b_c, c_c, d_c, e_c, T)
        correction_ratio = max_correction / max(amplitude, 1e-6)

        # Septic distortion must be below the septic limit (use effective limit if provided)
        septic_limit = effective_septic_distortion_limit if effective_septic_distortion_limit is not None else getattr(cfg, 'septic_distortion_limit', 1.0)
        if correction_ratio > septic_limit:
            return None  # Too distorting, fall back to quintic/cubic

        # Apply correction
        corrected = []
        for t, v in resampled:
            c_t = self._evaluate_septic_correction(t, a_c, b_c, c_c, d_c, e_c)
    ...
    def _detect_bone_chains(
        ...
        """v17: Detect bone chains from naming patterns.

        Returns dict mapping chain_prefix -> [sorted list of bone names in chain].
        """
        chain_groups = defaultdict(list)
        cfg = self.config
        chain_patterns = getattr(cfg, 'chain_naming_patterns', (
            r'taclejoint[BW][LR]?\d+',
            r'joint[DUF][BLR]\d+',
            r'hair_joint[LR]?\d+',
            r'tentacle\d+',
            r'jointLA\d+', r'jointRA\d+',
        ))

        for bone_name in bone_channels.keys():
            for pattern in chain_patterns:
                match = re.search(pattern, bone_name, re.IGNORECASE)
                if match:
                    # Extract the chain prefix (everything before the number)
                    prefix_match = re.match(r'^(.*?)(\d+)$', bone_name, re.IGNORECASE)
                    if prefix_match:
                        chain_prefix = prefix_match.group(1).rstrip('_')
                        chain_groups[chain_prefix].append(bone_name)
                    break

        # Filter to chains with at least min_length bones
        min_chain_len = getattr(cfg, 'bone_chain_min_length', 3)
        return {k: sorted(v) for k, v in chain_groups.items() if len(v) >= min_chain_len}

    def _apply_chain_aware_c1_correction(
        self,
        bone_channels: Dict[str, Dict[str, List[Tuple[float, float]]]],
        duration: float,
        interpolation: str,
        ...
        """v17: Chain-aware C1 correction — average C1 error across chain segments.

        For each detected chain, compute the average C1 error across all segments,
        then apply the average correction to each segment. This prevents one
        segment from having a different loop velocity than its neighbors.
        """
        if not chain_groups:
            return bone_channels

        for chain_prefix, bone_names in chain_groups.items():
            # Collect C1 errors for all channels in the chain
            c1_errors = []
            channel_data = []
            ...
            for bone_name in bone_names:
                ...
                for channel, keyframes in channels.items():
                    ...
                    n_resample = max(int(duration * self.config.resample_rate), 60)
                    ...
                    c1_error = vT - v0  # signed error
                    c1_errors.append(c1_error)
                    channel_data.append((bone_name, channel, keyframes, resampled, p0, v0, vT, resample_dt))

            if not c1_errors:
                continue

            # Compute average C1 error across the chain
            avg_c1_error = sum(c1_errors) / len(c1_errors)

            # Apply the average correction to each channel in the chain
            for bone_name, channel, keyframes, resampled, p0, v0, vT, resample_dt in channel_data:
                original_c1_error = vT - v0
                # The correction adjusts the velocity to use the chain-average error
                # rather than the individual error. This means each channel's
                # correction is: (avg_error - original_error) applied at the end.
                c1_adjustment = avg_c1_error - original_c1_error

                if abs(c1_adjustment) < 0.1:  # Skip if adjustment is negligible
                    continue

                # Apply a smooth correction over the last 25% of the animation
                n = len(resampled)
                zone_start_idx = max(1, n * 3 // 4)
                T_zone = resampled[-1][0] - resampled[zone_start_idx][0]

                if T_zone < 1e-8:
                    continue

                corrected = list(resampled)
                for i in range(zone_start_idx, n):
                    t_i, v_i = corrected[i]
                    s_i = (t_i - resampled[zone_start_idx][0]) / T_zone
                    s_i = max(0.0, min(1.0, s_i))

                    # Cubic correction: adjust velocity by c1_adjustment at the end
                    # c(s) = A*s^3 + B*s^2, c(0)=0, c(1)=0, c'(1)*T_zone = c1_adjustment
                    # c'(s)/T_zone = (3A*s^2 + 2B*s)/T_zone
                    # c'(1) = (3A + 2B)/T_zone = c1_adjustment
                    # A + B = 0 => B = -A
                    # (3A - 2A)/T_zone = c1_adjustment => A/T_zone = c1_adjustment
                    A_c = c1_adjustment * T_zone
                    B_c = -A_c
                    correction = A_c * s_i**3 + B_c * s_i**2
                    corrected[i] = (t_i, v_i + correction)

                # Ensure C0 is maintained
    ...
    def enforce(
        ...
        if getattr(cfg, 'chain_aware_c1_correction', True):
            ...
            if chain_groups:
                bone_channels = self._apply_chain_aware_c1_correction(
                    bone_channels, duration, interpolation, chain_groups
                )
                blend_diag['chain_aware_c1_applied'] = True
                blend_diag['chain_groups_detected'] = len(chain_groups)
                blend_diag['chain_details'] = {
    ...
    def enforce_with_full_resample(
        ...
        for bone_name, channels in bone_channels.items():
            for channel, keyframes in channels.items():
                ...
                # v17 NEW: Try septic global correction in full resample path
                # This provides C0+C1+C2+C3 continuity with smoother velocity transitions
                if getattr(cfg, 'septic_global_correction', True):
                    a0_rs = 0.0
                    aT_rs = 0.0
                    if len(resampled) >= 5:
                        a0_rs = (2*resampled[0][1] - 5*resampled[1][1] + 4*resampled[2][1] - resampled[3][1]) / (resample_dt * resample_dt)
                        aT_rs = (2*resampled[-1][1] - 5*resampled[-2][1] + 4*resampled[-3][1] - resampled[-4][1]) / (resample_dt * resample_dt)
                    ...
                    septic_corrected = self._apply_septic_global_correction(
                        resampled, delta_p, delta_v, delta_a, duration, amplitude, cfg
                    )
                    if septic_corrected is not None:
                        new_keyframes = self._rebuild_keyframes_from_resampled(
                            keyframes, septic_corrected, duration, p0
                        )
                        bone_channels[bone_name][channel] = new_keyframes
                        blend_diag['septic_correction_count'] = blend_diag.get('septic_correction_count', 0) + 1
                        continue  # Septic handled this channel
...
class BBModelAnimationConverter:
    ...
    def convert_file(self, bbmodel_path: str,
        ...
        for anim_name, anim_data in extracted['animations'].items():
            ...
            # v17: Skip static idle if model has other meaningful animations
            if getattr(cfg, 'consolidate_static_idles', True) and is_static:
                # Check if we have other meaningful (non-static) animations in this model
                meaningful_anims = [
                    a_name for a_name, a_data in extracted['animations'].items()
                    if a_name != anim_name
                    and not a_data.get('should_skip', False)
                    and not a_data.get('static', False)
                ]
                if meaningful_anims:
                    # Don't process this static idle - the model has real animations
                    stats['static_idle_skipped'] = True
                    continue
            ...
            original_duration = current_duration  # v17: save for evolved duration protection
            if loop_mode == "loop" and self.config.enable_duration_optimization:
                ...
                # If this is a walk animation with good C0 at original duration,
                # don't extend it — extending creates more problems than it solves
                is_walk_for_duration = any(p in anim_name.lower() for p in ('walk', 'run'))
                if (is_walk_for_duration and
                    getattr(cfg, 'walk_preserve_original_duration', True) and
                    current_c0 < getattr(cfg, 'walk_duration_extend_c0_threshold', 1.0)):
                    # Walk already has acceptable C0 at original duration — don't extend
                    should_change = False
                    if method in ('search_optimal', 'search_early_exit_good_enough',
                                  'search_optimal_c1_priority'):
                        duration_change_reason = (
                            f"walk C0 preserved: {current_c0:.3f} < "
                            f"{getattr(cfg, 'walk_duration_extend_c0_threshold', 1.0):.1f}deg"
                        )
                elif method in ('search_optimal', 'search_early_exit_good_enough',
                ...
                if should_change and abs(optimal_duration - current_duration) > 0.005:
                    # v17 NEW: Evolved duration anomaly fix
                    # Prevent evolved animations from being shortened below 60% of original
                    evolved_min_ratio = getattr(cfg, 'evolved_min_duration_ratio', 0.60)
                    if 'evolved' in anim_name.lower():
                        if optimal_duration < original_duration * evolved_min_ratio:
                            should_change = False
                            duration_change_reason = (
                                f"evolved duration protected: {optimal_duration:.2f}s < "
                                f"{evolved_min_ratio*100:.0f}% of {original_duration:.2f}s"
                            )
                            stats['evolved_duration_protected'] = stats.get('evolved_duration_protected', []) + [anim_name]
                    ...
                    if should_change:
                        ...
                        # v18 NEW: If duration is being EXTENDED (not shortened), and this
                        # is a walk animation, replicate the keyframes instead of just
                        # trimming (which would leave the extended portion as extrapolation)
                        is_walk_anim = any(p in anim_name.lower() for p in ('walk', 'run'))
                        if (optimal_duration > current_duration and is_walk_anim and
                            getattr(cfg, 'walk_replicate_on_duration_extend', True)):
                            ratio = optimal_duration / current_duration
                            if abs(ratio - round(ratio)) < 0.15 and round(ratio) >= 2:
                                # Duration is being extended to an integer multiple
                                n_cycles = round(ratio)
                                bone_channels = self._replicate_walk_keyframes(
                                    bone_channels, current_duration, n_cycles,
                                    use_periodic_extrapolation=False  # v20: skip cosine smoothing, use exact values
                                )
                                stats['walk_keyframes_replicated'] = stats.get('walk_keyframes_replicated', 0) + 1
                            else:
                                # Non-integer extension, just trim
                                bone_channels = self._trim_to_duration(bone_channels, optimal_duration)
                        else:
                            bone_channels = self._trim_to_duration(bone_channels, optimal_duration)
                        current_duration = optimal_duration
                    else:
                        if duration_change_reason:
                            stats['duration_adjustments'].append({
                                'animation': anim_name,
                                'from': current_duration,
                                'to': current_duration,
                                'method': 'evolved_protected',
                                'reason': duration_change_reason,
                            })
                else:
                    current_duration = anim_data['length']
            ...
            # v20: Walk cycle extension for too-fast walks MOVED to after C1 enforcement
            # (see below, after Step 9c)
            ...
            if loop_mode == "loop":
                ...
                # v18 NEW: Walk-specific lightweight C1 enforcement
                # Walk animations benefit from a much lighter C1 approach that
                # preserves the natural sinusoidal structure of leg motion
                is_walk_for_c1 = any(p in anim_name.lower() for p in ('walk', 'run'))
                use_walk_lightweight_c1 = (is_walk_for_c1 and
                    getattr(cfg, 'walk_lightweight_c1', True))

                if use_walk_lightweight_c1 and hasattr(self, '_walk_lightweight_c1_enforcement'):
                    bone_channels, blend_diag = self._walk_lightweight_c1_enforcement(
                        bone_channels, current_duration, interpolation,
                        cached_resampled=cached_resampled,
                        anim_name=anim_name,
                    )
                elif hasattr(self.c1_enforcer, 'enforce_with_full_resample'):
            ...
            # v20 NEW: Walk cycle extension for too-fast walks — MOVED from before C1
            # enforcement. In v19, this ran BEFORE C1 enforcement, and the aggressive
            # C1 enforcement then created artifacts in the replicated data. In v20,
            # we first apply gentle C1 correction on the single cycle, THEN extend,
            # so the replicated cycle inherits the corrected velocity match.
            is_walk_anim = any(p in anim_name.lower() for p in ('walk', 'run'))
            if (is_walk_anim and current_duration < 0.8 and loop_mode == "loop"
                and getattr(cfg, 'walk_cycle_extension_enabled', True)):
                # Check if current cycle has good C0 (is complete)
                c0_ok = True
                for bone_name, channels in bone_channels.items():
                    for ch_name, kfs in channels.items():
                        if len(kfs) >= 2:
                            first_v = kfs[0][1]
                            last_v = kfs[-1][1]
                            if abs(last_v - first_v) > 0.5:  # 0.5 deg tolerance
                                c0_ok = False
                                break
                    if not c0_ok:
                        break
                ...
                if c0_ok:
                    # Double the walk cycle for better pacing
                    extension_factor = 2
                    new_duration = current_duration * extension_factor
                    # Snap to tick boundary
                    new_duration = round(new_duration * 20.0) / 20.0
                    if new_duration > current_duration:
                        # Replicate keyframes using v20 improved method
                        bone_channels = self._replicate_walk_keyframes(
                            bone_channels, current_duration, extension_factor,
                            use_periodic_extrapolation=False  # v20: skip cosine smoothing, use exact values
                        )
                        duration_change_reason = (
                            f"v20 walk extended: {current_duration:.4f}s -> {new_duration:.4f}s "
                            f"(factor={extension_factor}, too-fast humanoid walk, after gentle C1)"
                        )
                        stats['walk_cycle_extended'] = stats.get('walk_cycle_extended', [])
                        stats['walk_cycle_extended'].append({
                            'animation': anim_name,
                            'from': current_duration,
                            'to': new_duration,
                            'factor': extension_factor,
                        })
                        current_duration = new_duration

                    # v20: Final C0 snap after walk extension
                    for bone_name, channels in bone_channels.items():
                        for ch_name, kfs in channels.items():
                            if len(kfs) >= 2:
                                first_v = kfs[0][1]
                                last_v = kfs[-1][1]
                                if abs(last_v - first_v) > 0.05:
                                    kfs[-1] = (kfs[-1][0], first_v)

                    # v20: Spike smoothing after walk cycle extension
                    bone_channels = self._walk_spike_smoothing(
                        bone_channels, current_duration
                    )

            # v17 NEW: Walk Body Motion Synthesis for degenerate walks
            # For walk animations with ≤4 leg bones, synthesize body sway
            walk_body_sway_synthesized = False
            walk_body_bob_added = False
            is_walk_anim = any(p in anim_name.lower() for p in ('walk', 'run'))
            if (loop_mode == "loop" and not is_static and is_walk_anim and
                getattr(cfg, 'walk_body_motion_synthesis', True)):
                bone_channels, walk_body_sway_synthesized, walk_body_bob_added = \
                    self._synthesize_walk_body_motion(
                        bone_channels, current_duration, interpolation, anim_name
                    )

            # v19 NEW: Synthesize motion for bones missing from walk
            is_walk_for_synthesis = any(p in anim_name.lower() for p in ('walk', 'run'))
            if is_walk_for_synthesis and getattr(cfg, 'walk_missing_bone_synthesis', True):
                bone_channels = self._synthesize_walk_missing_bones(
                    bone_channels, current_duration, extracted['animations'], anim_name
                )
            ...
            if (loop_mode == "loop" and not is_static and is_walk_anim and
                ...
                if walk_kf_generated > 0:
                    ...
                    # v20 NEW: Spike smoothing after walk validation resample
                    # Periodic resampling can create velocity spikes from close-together
                    # original keyframes. Smooth them out before C1 correction.
                    bone_channels = self._walk_spike_smoothing(
                        bone_channels, current_duration
                    )
        ...
        # v18 NEW: Exact-Duplicate Animation Consolidation
        # After all C1/C2 enforcement, detect animations with IDENTICAL output
        # data and consolidate them. This handles ferCow-style models where
        # idle/evolved/attack are exactly the same after processing.
        if getattr(cfg, 'exact_duplicate_consolidation', True):
            all_animations, quality_reports, consolidated = self._exact_duplicate_consolidation(
                all_animations, quality_reports
            )
            if consolidated:
                stats['exact_duplicates_consolidated'] = consolidated
                for cname in consolidated:
                    stats['total_animations'] -= 1
        ...
        # v17 NEW: Smart static idle consolidation — if model has ONLY static idle
        # (all-zero root position), mark it in the registry instead of writing the file
        is_only_static_idle = False
        if (getattr(cfg, 'generate_static_idle_for_empty_models', True) and
            not all_animations and model_name):
            # Check if this model has any other meaningful animations at all
            # (If it does, we skip generating static idle — v17 improvement)
            # v17 FIX: Start with False, only set True if we find non-skipped, non-static anims
            has_other_animations = False
            for anim_name_chk, anim_data_chk in extracted.get('animations', {}).items():
                if not anim_data_chk.get('should_skip', False) and not anim_data_chk.get('static', False):
                    has_other_animations = True
                    break

            if has_other_animations and getattr(cfg, 'consolidate_static_idles', True):
                # Model has other animations — don't generate static idle
                stats['static_idle_skipped'] = True
                is_only_static_idle = False
            else:
                # Model has NO other animations — generate static idle
                ...
                is_only_static_idle = True

                # v17: Register in the shared static idle registry
                if getattr(cfg, 'consolidate_static_idles', True):
                    static_idle_content = json.dumps(static_idle, sort_keys=True)
                    content_hash = hashlib.sha256(static_idle_content.encode()).hexdigest()
                    cfg.static_idle_registry[model_name] = {
                        'hash': content_hash,
                        'only_static_idle': True,
                    }
        ...
        # v17 NEW: Cross-model idle dedup — if this file's content is identical
        # to another already-written file, skip writing it
        if output_path:
            should_write = True
            skip_reason = ""
            ...
            # v17: If this model has ONLY static idle, check if another model
            # already wrote an identical file — if so, skip writing
            if is_only_static_idle and getattr(cfg, 'consolidate_static_idles', True):
                result_json = json.dumps(result, sort_keys=True)
                content_hash = hashlib.sha256(result_json.encode()).hexdigest()
                if hasattr(cfg, 'consolidated_file_registry'):
                    registry = cfg.consolidated_file_registry
                    # Check if this hash already exists
                    existing_model = registry.get(content_hash)
                    if existing_model:
                        should_write = False
                        skip_reason = f"duplicate_of_{existing_model}"
                        stats['files_skipped_all_empty'] = True
                        stats['cross_model_consolidated'] = True
                        # Track the consolidation for the ZIP packager
                        registry[model_name] = {
                            'consolidated_with': existing_model,
                            'hash': content_hash,
                            'original_path': output_path,
                        }
                    else:
                        # First occurrence — write it and register
                        registry[content_hash] = model_name
                        registry[model_name] = {
                            'hash': content_hash,
                            'is_primary': True,
                        }
    ...
    def _deep_idle_dedup(
        ...
        """v11/v17 Improvement 3: Aggressive Deep Idle Deduplication.
        ...
        # v17 NEW: Step 6 — Content-hash dedup across idle animations
        # For idle animations with identical content (same bone channels, same values),
        # keep only one and mark the others as consolidated
        if getattr(cfg, 'cross_model_file_consolidation', True) and len(idle_like) > 1:
            idle_hashes = {}
            for anim_name, anim_data in all_animations.items():
                name_lower = anim_name.lower()
                is_idle = ('idle' in name_lower or
                          any(alias in name_lower for alias in idle_aliases))
                if not is_idle:
                    continue

                # Compute content hash for this animation
                anim_content = json.dumps(anim_data.get('bones', {}), sort_keys=True)
                content_hash = hashlib.sha256(anim_content.encode()).hexdigest()

                if content_hash in idle_hashes:
                    # Duplicate found — remove this one
                    duplicate_name = anim_name
                    primary_name = idle_hashes[content_hash]
                    if duplicate_name in all_animations:
                        del all_animations[duplicate_name]
                    if duplicate_name in quality_reports:
                        quality_reports[duplicate_name].idle_dedup_deep_merged = True
                        quality_reports[duplicate_name].static_idle_consolidated = True
                        del quality_reports[duplicate_name]
                    if duplicate_name not in removed:
                        removed.append(duplicate_name)
                else:
                    idle_hashes[content_hash] = anim_name
    ...
    def _general_c1_post_correction(
        ...
        """v16/v17: General C1 Post-Correction for ALL loop animations.
    ...
    # ========================================================================
    # v17 NEW METHODS: Walk Body Motion Synthesis, Septic integration in
    # enforce_with_full_resample, and enhanced post-correction
    # ========================================================================

    def _synthesize_walk_body_motion(
        self,
        bone_channels: Dict[str, Dict[str, List[Tuple[float, float]]]],
        duration: float,
        interpolation: str,
        anim_name: str
    ) -> Tuple[Dict[str, Dict[str, List[Tuple[float, float]]]], bool, bool]:
        """v17: Synthesize body sway for degenerate walk animations.

        For walks with ≤4 leg bones (degenerate walks like rathol, done),
        add a subtle body bob (mainbody y-position oscillation) at 2x walk
        frequency and body rotation sway opposite to leg phase.

        Returns:
            (updated_bone_channels, sway_synthesized, bob_added)
        """
        cfg = self.config
        sway_synthesized = False
        bob_added = False
        ...
        for bone_name in bone_channels:
            bone_lower = bone_name.lower()
            if any(p in bone_lower for p in walk_patterns):
                leg_bones.append(bone_name)

        degenerate_threshold = getattr(cfg, 'walk_degenerate_leg_threshold', 4)
        if len(leg_bones) > degenerate_threshold:
            return bone_channels, False, False

        # Find mainbody or body bone for adding sway
        body_bone = None
        body_candidates = ('mainbody', 'body', 'torso', 'chest', 'root')
        for bone_name in bone_channels:
            bone_lower = bone_name.lower()
            for candidate in body_candidates:
                if candidate in bone_lower:
                    body_bone = bone_name
                    break
            if body_bone:
                break
        ...
        # Detect walk period from leg motion
        walk_period = duration  # default: full animation is one cycle
        if leg_bones:
            # Use the first leg bone's rotation to detect period
            for leg_bone in leg_bones[:2]:
                for channel in ('ry', 'rz'):
                    if channel in bone_channels.get(leg_bone, {}):
                        kfs = bone_channels[leg_bone][channel]
                        if len(kfs) >= 4:
                            ...
                            for i in range(1, len(values)):
                                if values[i-1] * values[i] < 0:
                                    zero_crossings.append((kfs[i-1][0] + kfs[i][0]) / 2.0)
                            if len(zero_crossings) >= 2:
                                # Half-period between consecutive crossings
                                half_periods = [zero_crossings[i+1] - zero_crossings[i]
                                               for i in range(len(zero_crossings)-1)]
                                if half_periods:
                                    walk_period = 2.0 * (sum(half_periods) / len(half_periods))
                                    break
                if walk_period != duration:
                    break

        bob_amplitude = getattr(cfg, 'walk_body_bob_amplitude', 0.3)
        sway_amplitude = getattr(cfg, 'walk_body_sway_amplitude', 0.5)

        # Add body bob (y-position oscillation at 2x walk frequency)
        if body_bone in bone_channels:
            ...
            if 'y' not in channels:
                ...
                # Snap last to first for C0
                if bob_kfs:
                    bob_kfs[-1] = (bob_kfs[-1][0], bob_kfs[0][1])

                channels['y'] = bob_kfs
                bob_added = True

            # Add body rotation sway (opposite to leg phase)
            for sway_channel in ('ry',):
                if sway_channel not in channels:
                    ...
                    # Snap last to first for C0
                    if sway_kfs:
                        sway_kfs[-1] = (sway_kfs[-1][0], sway_kfs[0][1])

                    channels[sway_channel] = sway_kfs
                    sway_synthesized = True

        return bone_channels, sway_synthesized, bob_added

    def _synthesize_walk_missing_bones(
        self,
        walk_bone_channels: Dict[str, Dict[str, List[Tuple[float, float]]]],
        walk_duration: float,
        all_animations_raw: Dict[str, Any],
        anim_name: str
    ) -> Dict[str, Dict[str, List[Tuple[float, float]]]]:
        """v20 UPDATED: Synthesize subtle motion for bones present in other animations
        but missing from the walk animation.
        
        For example, ferHuman's walk only animates leg/arm/body bones but not
        head, jaw, or back-ridge bones. This method adds subtle walk-synchronized
        motion for those missing bones.
        
        v20 FIX: Scale idle amplitudes by the ratio of walk's average amplitude
        to idle's average amplitude. If idle has a jointH rotation of 11 deg but
        walk's average leg amplitude is only 8 deg, scale the synthesized jointH
        rotation down proportionally. This ensures the synthesized motion matches
        the walk's energy level.
        
        Args:
            walk_bone_channels: Current walk animation bone channels
            walk_duration: Walk animation duration
            all_animations_raw: All raw animations from extraction (for bone reference)
            anim_name: Current animation name
            
        Returns:
            Updated bone_channels with synthesized missing bone motion
        """
        # Find bones that are in idle/other animations but NOT in walk
        all_bones_in_model = set()
        for other_name, other_data in all_animations_raw.items():
            if other_name == anim_name:
                continue
            other_bc = other_data.get('bone_channels', {})
            all_bones_in_model.update(other_bc.keys())
        
        walk_bones = set(walk_bone_channels.keys())
        missing_bones = all_bones_in_model - walk_bones
        
        if not missing_bones:
            return walk_bone_channels
        
        # v20 NEW: Compute walk and idle average amplitudes for proportional scaling
        walk_avg_amplitude = 0.0
        walk_amplitude_count = 0
        for bone_name, channels in walk_bone_channels.items():
            for ch, kfs in channels.items():
                if kfs:
                    vals = [abs(v) for t, v in kfs]
                    max_amp = max(vals) if vals else 0.0
                    walk_avg_amplitude += max_amp
                    walk_amplitude_count += 1
        if walk_amplitude_count > 0:
            walk_avg_amplitude /= walk_amplitude_count
        
        idle_avg_amplitude = 0.0
        idle_amplitude_count = 0
        for other_name, other_data in all_animations_raw.items():
            if 'idle' not in other_name.lower():
                continue
            other_bc = other_data.get('bone_channels', {})
            for bone_name, channels in other_bc.items():
                for ch, kfs in channels.items():
                    if kfs:
                        vals = [abs(v) for t, v in kfs]
                        max_amp = max(vals) if vals else 0.0
                        idle_avg_amplitude += max_amp
                        idle_amplitude_count += 1
            break  # Only use the first idle animation for reference
        
        if idle_amplitude_count > 0:
            idle_avg_amplitude /= idle_amplitude_count
        
        # v20: Compute the proportional scaling factor
        # If walk has lower average amplitude than idle, scale down the
        # synthesized amplitudes proportionally
        if idle_avg_amplitude > 0.01:
            walk_to_idle_ratio = walk_avg_amplitude / idle_avg_amplitude
        else:
            walk_to_idle_ratio = 1.0
        # Clamp the ratio to reasonable bounds
        walk_to_idle_ratio = max(0.2, min(2.0, walk_to_idle_ratio))
        
        # Classify missing bones
        head_patterns = ('jointh', 'head', 'neck')
        jaw_patterns = ('jointm', 'jaw', 'mouth')
        spine_patterns = ('jointrm', 'jointrr', 'jointrl', 'jointll', 'jointspine', 'spine')
        claw_patterns = ('claw', 'jointlac', 'jointrac', 'jointla1', 'jointra1')
        
        for bone_name in sorted(missing_bones):
            bone_lower = bone_name.lower()
            
            # Determine synthesis type based on bone name
            is_head = any(p in bone_lower for p in head_patterns)
            is_jaw = any(p in bone_lower for p in jaw_patterns)
            is_spine = any(p in bone_lower for p in spine_patterns)
            is_claw = any(p in bone_lower for p in claw_patterns)
            
            if not (is_head or is_jaw or is_spine or is_claw):
                continue  # Skip unknown bone types
            
            # Get the bone's idle animation for reference amplitude
            idle_amplitude = 0.0
            idle_channels = {}
            for other_name, other_data in all_animations_raw.items():
                if 'idle' not in other_name.lower():
                    continue
                other_bc = other_data.get('bone_channels', {})
                if bone_name in other_bc:
                    idle_channels = other_bc[bone_name]
                    # Find max amplitude
                    for ch, kfs in idle_channels.items():
                        if kfs:
                            vals = [abs(v) for t, v in kfs]
                            idle_amplitude = max(idle_amplitude, max(vals))
                    break
            
            # v20: Scale idle amplitudes by walk-to-idle ratio for proportional synthesis
            # This ensures the synthesized motion matches the walk's energy level
            scaled_idle_amplitude = idle_amplitude * walk_to_idle_ratio
            
            # Synthesize subtle walk-synchronized motion
            walk_period = walk_duration
            n_kfs = max(6, int(walk_duration / 0.08))
            
            new_channels = {}
            if is_head:
                # Head bob: subtle x-rotation at walk frequency, slight y-rotation sway
                # v20: Use scaled_idle_amplitude instead of idle_amplitude
                amplitude = min(scaled_idle_amplitude * 0.3, 3.0) if scaled_idle_amplitude > 0.5 else 1.5
                rx_kfs = []
                for i in range(n_kfs + 1):
                    t = round(i * walk_duration / n_kfs, 4)
                    rx = amplitude * math.sin(2.0 * math.pi * t / walk_period)
                    rx_kfs.append((t, round(rx, 6)))
                if rx_kfs:
                    rx_kfs[-1] = (rx_kfs[-1][0], rx_kfs[0][1])  # C0
                new_channels['rx'] = rx_kfs
                
                # Head sway: subtle z-rotation
                amplitude_z = min(scaled_idle_amplitude * 0.15, 1.5) if scaled_idle_amplitude > 0.5 else 0.5
                rz_kfs = []
                for i in range(n_kfs + 1):
                    t = round(i * walk_duration / n_kfs, 4)
                    rz = amplitude_z * math.cos(2.0 * math.pi * t / walk_period)
                    rz_kfs.append((t, round(rz, 6)))
                if rz_kfs:
                    rz_kfs[-1] = (rz_kfs[-1][0], rz_kfs[0][1])  # C0
                new_channels['rz'] = rz_kfs
                
            elif is_jaw:
                # Jaw: very subtle motion
                # v20: Use scaled_idle_amplitude
                amplitude = min(scaled_idle_amplitude * 0.2, 1.0) if scaled_idle_amplitude > 0.5 else 0.3
                rx_kfs = []
                for i in range(n_kfs + 1):
                    t = round(i * walk_duration / n_kfs, 4)
                    rx = amplitude * math.sin(2.0 * math.pi * t / walk_period + 0.5)
                    rx_kfs.append((t, round(rx, 6)))
                if rx_kfs:
                    rx_kfs[-1] = (rx_kfs[-1][0], rx_kfs[0][1])  # C0
                new_channels['rx'] = rx_kfs
                ...
                # Spine/ridge bones: subtle wave at walk frequency with phase offset
                # Detect spine segment index from name
                match = re.search(r'(\d+)', bone_name)
                seg_index = int(match.group(1)) if match else 0
                n_segs = 4  # typical number of segments
                
                phase_offset = seg_index * math.pi / (n_segs * 2)  # wave propagation
                
                # v20: Use scaled_idle_amplitude
                amplitude = min(scaled_idle_amplitude * 0.25, 2.0) if scaled_idle_amplitude > 0.5 else 0.8
                rx_kfs = []
                for i in range(n_kfs + 1):
                    t = round(i * walk_duration / n_kfs, 4)
                    rx = amplitude * math.sin(2.0 * math.pi * t / walk_period + phase_offset)
                    rx_kfs.append((t, round(rx, 6)))
                if rx_kfs:
                    rx_kfs[-1] = (rx_kfs[-1][0], rx_kfs[0][1])  # C0
                new_channels['rx'] = rx_kfs
                
                # Subtle z-rotation for lateral sway
                amplitude_z = amplitude * 0.5
                rz_kfs = []
                for i in range(n_kfs + 1):
                    t = round(i * walk_duration / n_kfs, 4)
                    rz = amplitude_z * math.cos(2.0 * math.pi * t / walk_period + phase_offset)
                    rz_kfs.append((t, round(rz, 6)))
                if rz_kfs:
                    rz_kfs[-1] = (rz_kfs[-1][0], rz_kfs[0][1])  # C0
                new_channels['rz'] = rz_kfs
                ...
                # Claw/arm extension bones: subtle z-rotation synchronized with arm swing
                # v20: Use scaled_idle_amplitude
                amplitude = min(scaled_idle_amplitude * 0.2, 1.5) if scaled_idle_amplitude > 0.5 else 0.5
                rz_kfs = []
                for i in range(n_kfs + 1):
                    t = round(i * walk_duration / n_kfs, 4)
                    rz = amplitude * math.sin(2.0 * math.pi * t / walk_period + math.pi/4)
                    rz_kfs.append((t, round(rz, 6)))
                if rz_kfs:
                    rz_kfs[-1] = (rz_kfs[-1][0], rz_kfs[0][1])  # C0
                new_channels['rz'] = rz_kfs
            ...
            if new_channels:
                walk_bone_channels[bone_name] = new_channels
    ...
    def _walk_validation_resample(
        ...
        """v20 Enhanced: Walk Validation — ensure ALL walk animations have sufficient keyframes.

        v20 Fix: Uses PERIODIC catmullrom resampling for sparse walk keyframes (3-5 KFs).
        The standard catmullrom interpolation falls back to linear for <4 KFs and creates
        loop boundary discontinuities even for 4+ KFs. The periodic version uses wrap-around
        tangent computation, creating smooth loop curves even for 3-keyframe walks.
        ...
        for bone_name, channels in bone_channels.items():
            ...
            for channel, keyframes in channels.items():
                ...
                # v20: Use PERIODIC resampling for walk channels
                # This is critical for sparse keyframes (3-5 KFs) where the standard
                # catmullrom falls back to linear or creates loop boundary discontinuities
                n_target = max(min_kfs, len(keyframes))
                dt = duration / n_target
                target_times = [i * dt for i in range(n_target + 1)]

                try:
                    # v20: Use periodic resampling for walks (smooth loop boundary)
                    if len(keyframes) <= 6:
                        resampled = CatmullRomEvaluator.resample_channel_periodic(
                            keyframes, target_times, interpolation
                        )
                    else:
                        resampled = CatmullRomEvaluator.resample_channel(
                ...
                # For periodic resampling, REPLACE keyframes entirely (don't mix old+new)
                # because the periodic resample produces a smoother curve than the sparse originals
                if len(keyframes) <= 6 and len(resampled) >= len(keyframes):
                    new_kfs = resampled
                else:
    ...
    def _walk_spike_smoothing(
        self,
        bone_channels: Dict[str, Dict[str, List[Tuple[float, float]]]],
        duration: float,
        max_velocity_ratio: float = 8.0
    ) -> Dict[str, Dict[str, List[Tuple[float, float]]]]:
        """v20 NEW: Remove velocity spikes from walk animation keyframes.

        After periodic resampling, walk animations can have velocity spikes
        where adjacent keyframe intervals have velocity ratios > 8x. These
        spikes occur when the original sparse keyframes had very close-together
        pairs (e.g., t=0.5278 and t=0.5334) representing rapid transitions.

        This method detects such spikes and smooths them by:
        1. Identifying keyframes where the velocity changes by > max_velocity_ratio
        2. Removing the spike keyframe if it's redundant (within 0.05s of another)
        3. Or adjusting its value to create a smoother transition

        Args:
            bone_channels: Walk animation bone channel data
            duration: Animation duration
            max_velocity_ratio: Maximum allowed velocity ratio between adjacent intervals

        Returns:
            Smoothed bone channel data
        """
        result = {}
        total_removed = 0
        ...
        for bone_name, channels in bone_channels.items():
            ...
            for channel, keyframes in channels.items():
                ...
                while changed and iterations < 5:
                    changed = False
                    iterations += 1
                    new_kfs = [sorted_kfs[0]]  # Always keep first

                    for i in range(1, n - 1):
                        dt_prev = sorted_kfs[i][0] - sorted_kfs[i-1][0]
                        dt_next = sorted_kfs[i+1][0] - sorted_kfs[i][0]

                        if dt_prev < 1e-12 or dt_next < 1e-12:
                            # Keep very close keyframes for now
                            new_kfs.append(sorted_kfs[i])
                            continue

                        v_prev = (sorted_kfs[i][1] - new_kfs[-1][1]) / dt_prev
                        v_next = (sorted_kfs[i+1][1] - sorted_kfs[i][1]) / dt_next

                        # Check for velocity spike
                        if abs(v_prev) > 0.1 and abs(v_next) > 0.1:
                            ratio = max(abs(v_next / v_prev), abs(v_prev / v_next))
                            sign_change = (v_prev * v_next) < 0

                            if (ratio > max_velocity_ratio and sign_change) or ratio > max_velocity_ratio * 3:
                                # This is a spike - check if removing it would be better
                                # Compute the velocity if we skip this keyframe
                                dt_skip = sorted_kfs[i+1][0] - new_kfs[-1][0]
                                if dt_skip > 1e-12:
                                    v_skip = (sorted_kfs[i+1][1] - new_kfs[-1][1]) / dt_skip
                                    # If skipping creates a smoother transition, remove this KF
                                    if abs(v_skip) < max(abs(v_prev), abs(v_next)) * 0.5:
                                        total_removed += 1
                                        changed = True
                                        continue  # Skip this keyframe

                                # If we can't remove it, smooth it by linear interpolation
                                alpha = dt_prev / (dt_prev + dt_next)
                                smoothed_val = new_kfs[-1][1] + alpha * (sorted_kfs[i+1][1] - new_kfs[-1][1])
                                # Blend between original and smoothed (70% smoothed)
                                new_val = sorted_kfs[i][1] * 0.3 + smoothed_val * 0.7
                                new_kfs.append((sorted_kfs[i][0], new_val))
                                continue

                        new_kfs.append(sorted_kfs[i])

                    new_kfs.append(sorted_kfs[-1])  # Always keep last

                    # Ensure C0 after smoothing
                    if len(new_kfs) >= 2:
                        if abs(new_kfs[0][1] - new_kfs[-1][1]) > 0.05:
                            new_kfs[-1] = (new_kfs[-1][0], new_kfs[0][1])
    ...
    def _walk_c1_correction_pass(
        ...
        """v16/v17: Walk-specific C1 correction pass with chain-aware refinement.
        ...
        # v17 NEW: Chain-aware C1 refinement for walk animations
        # After individual channel correction, detect leg chains and enforce
        # phase coherence across consecutive chain segments
        cfg = self.config
        if getattr(cfg, 'chain_aware_c1_correction', True):
            chain_groups = self.c1_enforcer._detect_bone_chains(bone_channels)
            if chain_groups:
                bone_channels = self.c1_enforcer._apply_chain_aware_c1_correction(
                    bone_channels, duration, interpolation, chain_groups
                )
                info['chain_aware_applied'] = True
                info['chain_groups'] = {k: len(v) for k, v in chain_groups.items()}
    ...
    def _is_truly_static_animation(
        self,
        anim_data: Dict[str, Any]
    ) -> bool:
        """v17: Check if an animation is truly static (all channels have near-zero motion).

        Used by static idle consolidation to distinguish meaningful animations
        from static placeholders.
        """
        cfg = self.config
        rot_thresh = cfg.truly_static_rot_threshold if hasattr(cfg, 'truly_static_rot_threshold') else 0.01
        pos_thresh = cfg.truly_static_pos_threshold if hasattr(cfg, 'truly_static_pos_threshold') else 0.001
        ...
        if anim_data.get('static', False):
            return True

        bones = anim_data.get('bones', {})
        if not bones:
            return True  # No bone data = effectively static
    ...
    def _replicate_walk_keyframes(
        self,
        bone_channels: Dict[str, Dict[str, List[Tuple[float, float]]]],
        cycle_duration: float,
        n_cycles: int,
        use_periodic_extrapolation: bool = True
    ) -> Dict[str, Dict[str, List[Tuple[float, float]]]]:
        """v20 UPDATED: Replicate walk keyframes for duration extension.

        When a walk animation's duration is extended to Nx, instead of just
        changing animation_length (which leaves the second half as extrapolation),
        this method replicates the keyframes N times with proper time offsets.

        v20 FIX: When replicating cycle 2+, use the ORIGINAL source keyframe
        values (not resampled or C1-corrected values that may have artifacts).
        The last keyframe of the final cycle exactly matches the first keyframe
        of cycle 1 for perfect C0 continuity.

        Args:
            bone_channels: Current bone channel data
            cycle_duration: Duration of one cycle (original duration)
            n_cycles: Number of times to replicate (e.g., 2 for 2x extension)
            use_periodic_extrapolation: If True, smooth the boundary between cycles
                (v20: default changed to False for better fidelity)

        Returns:
            New bone_channels with replicated keyframes and extended duration
        """
        result = {}
        ...
        for bone_name, channels in bone_channels.items():
            ...
            for channel, keyframes in channels.items():
                ...
                # v20: Store the original cycle 1 keyframes as the template.
                # These are the keyframes after gentle C1 correction but before
                # any resampling artifacts. Use exact values for replication.
                first_val = sorted_kfs[0][1]

                # Build replicated keyframes using ORIGINAL source values
                all_kfs = []

                for cycle in range(n_cycles):
                    time_offset = cycle * cycle_duration

                    # v20: For each cycle, use the exact same keyframe values as
                    # cycle 1, just offset in time. This preserves the original
                    # source keyframe values for better fidelity.
                    kfs_to_add = list(sorted_kfs)

                    if cycle < n_cycles - 1:
                        # Don't include the last KF of this cycle
                        # (it's the same as the first KF of next cycle)
                        # But DO include it if it's not at cycle_duration
                        if abs(kfs_to_add[-1][0] - cycle_duration) < 0.01:
                            kfs_to_add = kfs_to_add[:-1]

                    for t, v in kfs_to_add:
                        new_t = t + time_offset
                        # v20: Use the ORIGINAL keyframe value directly.
                        # No resampling, no cosine blending — just the exact
                        # same values as cycle 1, offset in time.
                        # Ensure we don't duplicate time points
                        if not all_kfs or abs(new_t - all_kfs[-1][0]) > 0.001:
                            all_kfs.append((new_t, v))

                # v20: The last keyframe of the extended animation MUST exactly
                # match the first keyframe value for perfect C0 continuity.
                if all_kfs:
                    total_duration = n_cycles * cycle_duration
                    last_kf = all_kfs[-1]
                    if abs(last_kf[0] - total_duration) < 0.01:
                        all_kfs[-1] = (total_duration, first_val)
                    else:
                        # Add a keyframe at total_duration matching the first value
                        all_kfs.append((total_duration, first_val))

                # v20: Skip periodic_extrapolation (cosine smoothing) by default.
                # The original source keyframe values are already smooth and correct.
                # Cosine blending at cycle boundaries can introduce artifacts.
                if use_periodic_extrapolation and len(all_kfs) > 6:
                    all_kfs = self._smooth_cycle_boundaries(
                        all_kfs, cycle_duration, n_cycles
                    )

                all_kfs.sort(key=lambda x: x[0])
                result[bone_name][channel] = all_kfs

        return result

    def _smooth_cycle_boundaries(
        self,
        keyframes: List[Tuple[float, float]],
        cycle_duration: float,
        n_cycles: int,
        ...
        """Smooth the boundaries between replicated cycles.

        Uses cosine blending in a small window around each cycle boundary
        to ensure smooth velocity transitions. The window is blend_ratio
        of the cycle duration (default 5%).

        Args:
            keyframes: All keyframes across all cycles
            cycle_duration: Duration of one cycle
            n_cycles: Number of cycles
            blend_ratio: Size of blend window as fraction of cycle duration

        Returns:
            Smoothed keyframes
        """
        if n_cycles <= 1 or len(keyframes) < 4:
            return keyframes

        result = list(keyframes)
        blend_window = cycle_duration * blend_ratio

        for cycle in range(1, n_cycles):
            boundary_time = cycle * cycle_duration
            blend_start = boundary_time - blend_window
            blend_end = boundary_time + blend_window

            # Find keyframes in the blend window
            blend_kfs = [(t, v) for t, v in result
                         if blend_start <= t <= blend_end]

            if len(blend_kfs) < 2:
                continue

            # Compute the value at the boundary from both sides
            left_val = None
            right_val = None
            for t, v in blend_kfs:
                if t < boundary_time:
                    left_val = v
                elif t > boundary_time:
            ...
            # Apply cosine blending to keyframes in the window
            new_result = []
            for t, v in result:
                if blend_start <= t <= blend_end:
                    # Normalized position in blend window [0, 1]
                    alpha = (t - blend_start) / (blend_end - blend_start)
                    alpha = max(0.0, min(1.0, alpha))
                    # Cosine interpolation weight
                    weight = 0.5 * (1.0 - math.cos(math.pi * alpha))
                    # Blend between left value and right value
                    blended_v = left_val * (1.0 - weight) + right_val * weight
                    new_result.append((t, blended_v))
                else:
                    new_result.append((t, v))
            result = new_result

        return result

    def _walk_lightweight_c1_enforcement(
        self,
        bone_channels: Dict[str, Dict[str, List[Tuple[float, float]]]],
        duration: float,
        interpolation: str,
        ...
        """v20 UPDATED: Gentle velocity-matching C1 enforcement for walk animations.

        v20 FIX: The v19 approach called enforce_with_full_resample() then applied
        a cosine bridge, which created velocity spikes (e.g., 8.9x ratio at loop
        boundary in ferHuman). The v20 approach is much simpler and gentler:

        1. First, ensure C0 continuity by snapping last keyframe to first
        2. Skip the full_resample C1 enforcement entirely for walk animations
        3. Instead, apply a GENTLE velocity-matching correction ONLY at the
           last keyframe:
           - For each channel, compute v_start (velocity between first 2 KFs)
             and v_end (velocity between last 2 KFs)
           - If |v_start - v_end| > 1.5 deg/s, adjust the SECOND-TO-LAST
             keyframe value to make the end velocity match the start velocity
           - Use linear interpolation: adjust penultimate KF by
             (v_end - v_start) * dt_end
        4. After this correction, re-verify C0 and snap if needed
        5. Key insight: walk animations with good C0 from source don't need
           aggressive C1 enforcement. A simple velocity match is sufficient.

        Returns:
            (corrected_bone_channels, blend_diagnostics)
        """
        cfg = self.config
        blend_diag = {
            'global_cubic_count': 0,
            ...
            'quintic_correction_count': 0,
            'quintic_hermite_zone_count': 0,
            'walk_lightweight_c1': True,  # marker
            'v20_gentle_velocity_match': True,  # v20 marker
        }

        # Step 1: Ensure C0 continuity by snapping last keyframe to first
        for bone_name, channels in bone_channels.items():
            for channel, keyframes in channels.items():
                if len(keyframes) < 2:
                    continue
                sorted_kfs = sorted(keyframes, key=lambda x: x[0])
                first_val = sorted_kfs[0][1]
                last_val = sorted_kfs[-1][1]
                c0_err = abs(last_val - first_val)
                if c0_err > 0.05:
                    # Snap last keyframe to first
                    sorted_kfs[-1] = (sorted_kfs[-1][0], first_val)
                    channels[channel] = sorted_kfs
                    blend_diag['static_snap_count'] = blend_diag.get('static_snap_count', 0) + 1

        # Step 2: v20 GENTLE velocity-matching correction
        # For each channel, if start and end velocities differ by > 1.5 deg/s,
        # adjust the penultimate keyframe to make the end velocity match start
        VELOCITY_MATCH_THRESHOLD = 1.5  # deg/s
        corrections_applied = 0

        for bone_name, channels in bone_channels.items():
            for channel, keyframes in channels.items():
                if len(keyframes) < 3:
                    # Need at least 3 keyframes to adjust the penultimate one
                    continue

                sorted_kfs = sorted(keyframes, key=lambda x: x[0])
                n = len(sorted_kfs)

                # Compute start velocity (between first 2 keyframes)
                dt_start = sorted_kfs[1][0] - sorted_kfs[0][0]
                if dt_start < 1e-12:
                    continue
                v_start = (sorted_kfs[1][1] - sorted_kfs[0][1]) / dt_start

                # Compute end velocity (between last 2 keyframes)
                dt_end = sorted_kfs[-1][0] - sorted_kfs[-2][0]
                if dt_end < 1e-12:
                    continue
                v_end = (sorted_kfs[-1][1] - sorted_kfs[-2][1]) / dt_end

                velocity_diff = v_end - v_start

                if abs(velocity_diff) <= VELOCITY_MATCH_THRESHOLD:
                    # Velocities already match well enough
                    continue

                # Adjust the penultimate keyframe value to make end velocity
                # match start velocity. We want:
                #   (last_val - new_penultimate_val) / dt_end = v_start
                # So: new_penultimate_val = last_val - v_start * dt_end
                last_val = sorted_kfs[-1][1]
                new_penultimate_val = last_val - v_start * dt_end

                # Apply the adjustment to the penultimate keyframe
                old_penultimate_val = sorted_kfs[-2][1]
                sorted_kfs[-2] = (sorted_kfs[-2][0], new_penultimate_val)
                corrections_applied += 1

                blend_diag['correction_magnitudes'].append(
                    abs(new_penultimate_val - old_penultimate_val)
                )

                channels[channel] = sorted_kfs

        blend_diag['v20_velocity_corrections'] = corrections_applied

        # Step 3: Re-verify C0 and snap if needed (the penultimate adjustment
        # shouldn't affect C0, but verify just in case)
        for bone_name, channels in bone_channels.items():
            for channel, keyframes in channels.items():
                if len(keyframes) < 2:
                    continue
    ...
    def _exact_duplicate_consolidation(
        self,
        all_animations: Dict[str, Any],
        quality_reports: Dict[str, Any]
    ) -> Tuple[Dict[str, Any], Dict[str, Any], List[str]]:
        """v20 UPDATED: Consolidate animations with identical output data.

        After all C1/C2 enforcement and quality processing, detect animations
        that have IDENTICAL bone channel data (same bones, same keyframes,
        same values). Keep only the most important one and remove the rest.

        v20 FIX: Consolidate ACROSS protected categories when content is
        identical. Priority order: idle > walk > attack > death > evolved > other.
        This ensures that for ferCow-style models where idle/evolved/attack are
        identical, we keep "idle" as the canonical name, and remove the duplicates
        regardless of their category.

        Args:
            all_animations: Dict of animation_name -> animation JSON data
            quality_reports: Dict of animation_name -> quality report

        Returns:
            (consolidated_animations, updated_reports, removed_names)
        """
        # Priority categories (lower number = higher priority = keep this one)
        PRIORITY = {
            'idle': 1,
            'walk': 2,
            'attack': 3,
        ...
        def _get_category_priority(name):
            name_lower = name.lower()
        ...
        # Compute content hash for each animation
        def _hash_animation(anim_data):
            """Hash the bone channel data of an animation."""
            bones = anim_data.get('bones', {})
            # Sort bones and channels for consistent hashing
            content_parts = []
            for bone_name in sorted(bones.keys()):
                bone_data = bones[bone_name]
                for channel_name in sorted(bone_data.keys()):
                    channel_data = bone_data[channel_name]
                    # Convert to a deterministic string
                    if isinstance(channel_data, dict):
                        # Axis-based format: {"x": {"0.0": 1.0, ...}, ...}
                        for axis in sorted(channel_data.keys()):
                            axis_data = channel_data[axis]
                            if isinstance(axis_data, dict):
                                items = sorted(axis_data.items(), key=lambda x: float(x[0]))
                                content_parts.append(f"{bone_name}.{channel_name}.{axis}={items}")
                            elif isinstance(axis_data, (int, float)):
                                content_parts.append(f"{bone_name}.{channel_name}.{axis}={axis_data}")
                    elif isinstance(channel_data, list):
                        content_parts.append(f"{bone_name}.{channel_name}={channel_data}")
            content_str = "|".join(content_parts)
            return hashlib.sha256(content_str.encode()).hexdigest()

        # Group animations by content hash
        hash_groups = {}  # hash -> [(anim_name, priority)]
        anim_hashes = {}  # anim_name -> hash

        for anim_name, anim_data in all_animations.items():
            h = _hash_animation(anim_data)
            anim_hashes[anim_name] = h
            pri = _get_category_priority(anim_name)
            if h not in hash_groups:
                hash_groups[h] = []
            hash_groups[h].append((anim_name, pri))

        # v20 FIX: Cross-category consolidation — when animations have identical
        # content hashes, ALWAYS consolidate regardless of protected category.
        # Rationale: if the source artist made 3 identical animations (e.g.,
        # idle/evolved/attack), there is no value in keeping all 3 — the game
        # can reference the single remaining animation for all states.
        # Keep the one with the highest priority (lowest priority number).
        ...
        for h, group in hash_groups.items():
            ...
            # v20: Sort ALL duplicates by priority (across categories) and keep
            # the one with the highest priority (lowest priority number)
            group.sort(key=lambda x: x[1])
            kept_name = group[0][0]
            kept_priority = group[0][1]

            # Determine which category the kept name belongs to for the note
            kept_category_str = 'other'
            for cat_name, pri in PRIORITY.items():
                if cat_name in kept_name.lower():
                    kept_category_str = cat_name
                    break

            for dup_name, _ in group[1:]:
                removed.append(dup_name)
                if dup_name in all_animations:
                    del all_animations[dup_name]
                if dup_name in quality_reports:
                    del quality_reports[dup_name]

            consolidated_names = [name for name, _ in group[1:]]
            if kept_name in all_animations:
                all_animations[kept_name]['_v20_consolidated_from'] = consolidated_names
                all_animations[kept_name]['_v20_consolidation_note'] = (
                    f"Cross-category consolidation: {', '.join(consolidated_names)} "
                    f"had identical data; kept as {kept_name} "
                    f"(category={kept_category_str}, priority={kept_priority}). "
                    f"The game can reference this single animation for all consolidated states."
                )
...
def batch_convert(input_dir: str, output_dir: str,
    ...
    """
    print("=" * 70)
    print("  Universal BBModel Animation Converter (v20)")
    print("  .bbmodel -> .animation.json with C1+C2 Correction & v20 Improvements")
    ...
    print()
    print("=" * 70)
    print("  DONE - Universal BBModel Animation Converter (v20)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Universal BBModel Animation Converter with C1+C2 Correction & v20 Improvements"
    )
