#!/usr/bin/env python3
"""
BBModelAnimationConverter - Universal Animation Converter (v7)
==============================================================
...
Key Improvements over v6:
  - CUBIC HERMITE TRANSITION ZONE: Replaces quintic Hermite with cubic Hermite
    in the transition zone for guaranteed C0+C1 continuity at BOTH the zone
    start boundary AND the loop boundary. No more velocity damping for bounce
    cases — the original velocity at the zone start is preserved exactly.
  - ENHANCED WALK CYCLE DETECTION: More bone name patterns (leg, foot, thigh,
    shin, knee, arm, hand, wing), better half-cycle mirroring with
    mean-shifted rotation and time-reversed position, endpoint matching.
  - IDLE ANIMATION SMART DEDUPLICATION: New idle name aliases (rest, breathing,
    ambient, pose), merge with more keyframes, idle_enriched logging.
  - SMART EMPTY ANIMATION ELIMINATION: Static marking, skip_meaningless option,
    minimal static pose for near-empty animations.
  - AUTO-LOOP WITH VELOCITY MATCHING: ±1 tick refinement, weighted C1 scoring,
    velocity zero-crossing heuristic for walk loops.
  - TEXTURE MAPPING UV FIX: Post-process geo.json to clamp UV bounds and fix
    negative uv_size on up/down faces.
  - QUALITY SCORING ADJUSTMENTS: Periodic bonus for c1_method=='none',
    harsher bounce penalty (-15), score ENHANCED version.
  - CONFIG UPDATES: New v7 fields for cubic Hermite, walk bone patterns,
    velocity zero-crossing, idle aliases, updated scoring weights.

Inherited from v6:
  - TRANSITION ZONE BLEND (now with cubic Hermite for C0+C1 guarantee)
  - PERIODIC ANIMATION ENHANCER (with improved walk cycle detection)
...
class ConverterConfig:
    """Master configuration for BBModelAnimationConverter v7."""
    # --- Loop Detection ---
...
class AnimationQualityReport:
    """Quality metrics for a single animation (v7 enhanced)."""
    animation_name: str
    duration: float
    num_bones: int
    total_keyframes: int
...
# ============================================================================
# Auto Loop Detector (v7: velocity-weighted scoring + ±tick refinement)
# ============================================================================
...
# ============================================================================
# Main Converter (v7: cubic Hermite, UV fix, smart empty elimination)
# ============================================================================

class BBModelAnimationConverter:
    """Universal animation converter for .bbmodel files (v7).
    ...
    Pipeline:
      ...
      3. For periodic animations: detect and enhance periodicity
      4. For loop animations: detect optimal loop duration (velocity-weighted scoring)
      5. Enforce C1 continuity at loop boundaries (cubic Hermite transition zone)
      6. Simplify keyframes
      7. Build GeckoLib .animation.json
      8. Smart empty animation elimination
      9. Post-process geo.json UV mapping (v7 NEW)
     10. Generate quality report (score ENHANCED version)

    v7 Improvements:
      - Cubic Hermite Transition Zone: guaranteed C0+C1 at BOTH boundaries
      - No velocity damping for bounce cases
      - Enhanced walk cycle detection and mirroring
      - Idle animation smart deduplication
      - Smart empty animation elimination
      - Velocity-weighted duration scoring (±tick refinement)
      - UV mapping post-processing
      - Quality scoring: periodic bonus, harsher bounce, score ENHANCED

    Inherited from v6:
      - Transition Zone Blend (now cubic Hermite)
      - Periodic Animation Enhancer
      - Global Cubic C1 Correction
      - Hybrid C1 Enforcer
      - Per-channel adaptive blend window
      - Enhanced empty animation consolidation
      - Enhanced duration optimization with harmonic search
    ...
    def convert_file(self, bbmodel_path: str,
        ...
        for anim_name, anim_data in extracted['animations'].items():
            ...
            # Step 3.5: Periodic Animation Enhancement
            periodicity_info = {
                'is_periodic': False,
                'period': None,
                'periodicity_score': 0.0,
            ...
            # Step 4: C1 continuity enforcement with cubic Hermite transition zone
            blend_diag = {
                'global_cubic_count': 0,
                'local_blend_count': 0,
                'static_snap_count': 0,
            ...
            # Step 6: Quality report — v7: score the ENHANCED version
            ...
            if periodic_enhanced:
                qreport.original_animation_preserved = 0.7
        ...
        # v7 Step 7: Smart Empty Animation Elimination
        all_static = True
        ...
        if output_path:
            # v7: Smart empty animation file handling
            ...
            elif all_truly_empty:
                # All animations are truly empty (no bone data)
                if not self.config.skip_meaningless_animation_files:
                    os.makedirs(os.path.dirname(output_path) if os.path.dirname(output_path) else '.', exist_ok=True)
                    with open(output_path, 'w', encoding='utf-8') as f:
                        json.dump(result, f, indent=2, ensure_ascii=False)
