#!/usr/bin/env python3
"""
BBModelAnimationConverter - Universal Animation Converter (v18)
==============================================================
...
class BBModelAnimationConverter:
    ...
    def _exact_duplicate_consolidation(
        ...
        # For each group with duplicates, keep the highest-priority one
        removed = []
        for h, group in hash_groups.items():
            ...
            kept_name = group[0][0]
            kept_category = [cat for cat, pri in PRIORITY.items() if pri == group[0][1]]
            kept_category_str = kept_category[0] if kept_category else 'other'
            ...
            # Add consolidation note to kept animation
            if len(group) > 1:
                consolidated_names = [name for name, _ in group[1:]]
                if kept_name in all_animations:
                    all_animations[kept_name]['_v18_consolidated_from'] = consolidated_names
                    all_animations[kept_name]['_v18_consolidation_note'] = (
                        f"Consolidated from duplicate animations: {', '.join(consolidated_names)} "
