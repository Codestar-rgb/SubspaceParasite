#!/usr/bin/env python3
"""
BBModelAnimationConverter - Universal Animation Converter (v9)
==============================================================
...
Key Improvements over v8 (7 major improvements):
  1. C2 ACCELERATION CONTINUITY AT LOOP BOUNDARIES: Quintic Hermite
     interpolation in transition zone for C0+C1+C2 continuity. Uses
     6 constraints: p(0), v(0), a(0), p(1), v(1), a(1). Computes
     acceleration from 2nd derivative of resampled data. Falls back to
     cubic Hermite if quintic distortion exceeds limit.
  2. WALK CYCLE FULL RECONSTRUCTION: Detects incomplete walks regardless
     of keyframe count by checking if leg rotation oscillation completes
     a full cycle (first_val ≈ last_val). Half-cycles are mirrored.
     Complete walks with poor loop continuity get end-point snapping
     and second-to-last keyframe adjustment for smooth C1 transition.
  3. DEEP IDLE DEDUPLICATION: Beyond empty-idle removal, detects and
     merges NEAR-DUPLICATE idle animations using content signatures
     (bone overlap + channel amplitude). Static idles (all channels
     < 0.05°) are consolidated. Multi-part entity awareness.
  4. ANIMATION FILE CONSOLIDATION: When the same model has multiple
     animation files from separate .bbmodel parts, consolidates them
     by merging same-named animation clips with union of bone channels.
  5. SMART ANIMATION TRUNCATION: Detects nearly-static trailing portion
     of animations and truncates at the point where meaningful motion
     ends, eliminating trailing static frames that cause loop discontinuity.
  6. ENHANCED C1 WITH QUINITIC GLOBAL CORRECTION: Extends global cubic
     correction to a quintic polynomial that matches acceleration at
     boundaries (6 constraints → C0+C1+C2). Falls back to cubic when
     distortion would be excessive.
  7. MULTI-TEXTURE EXTRACTION: Extracts ALL textures from .bbmodel files
     that have multiple textures (body + eyes + emissive), naming them
     model_name.png, model_name_1.png, model_name_2.png, etc.
...
class ConverterConfig:
    ...
    c2_distortion_limit: float = 0.40              # max correction/amplitude before falling back to cubic
    ...
    quintic_distortion_limit: float = 0.35         # max correction/amplitude before falling back to cubic
        ...
        for bone_name, channels in bone_channels.items():
            for channel, keyframes in channels.items():
                ...
                if not quintic_used and correction_ratio <= cfg.global_cubic_distortion_limit:
                    ...
                    # ====================================================
                    # TRANSITION ZONE BLEND — v9: C2 quintic or v7 cubic Hermite
                    # ====================================================
                    blend_diag['local_blend_count'] += 1
                    blend_diag['bridge_used_count'] += 1

                    if cfg.transition_zone_c2_hermite and hasattr(self, '_apply_transition_zone_blend_c2'):
                        corrected = self._apply_transition_zone_blend_c2(
                            resampled, duration, p0, v0, a0, vT, aT,
                    ...
                    blend_diag['bridge_details'].append({
                        ...
                        'method': 'transition_zone_blend_c2_quintic' if cfg.transition_zone_c2_hermite else 'transition_zone_blend_cubic_hermite',
                    })

                elif not quintic_used:
                    # ====================================================
                    # TRANSITION ZONE BLEND — v9: C2 quintic or v7 cubic Hermite (non-bounce)
                    # ====================================================
                    blend_diag['local_blend_count'] += 1
...
class BBModelAnimationConverter:
    ...
    def convert_file(self, bbmodel_path: str,
        ...
        # Step 13 [v9 NEW]: Deep idle dedup (near-duplicate + static consolidation)
    ...
    def _detect_and_reconstruct_walk_full_cycle(
        ...
        """v9 Improvement 2: Walk Cycle Full Reconstruction.
        ...
        b) If not, it's a half-cycle → mirror it
        ...
        # Step 1: Check if leg rotation channels complete a full cycle
        is_half_cycle = False
        is_full_cycle = True
        ...
        # Step 2b: Full cycle but poor loop continuity → snap end to start
        if is_full_cycle:
            result_channels = {}
            for bone_name, channels in bone_channels.items():
                result_channels[bone_name] = {}
    ...
    def _deep_idle_dedup(
        ...
        """v9 Improvement 3: Deep idle deduplication.

        Beyond empty-idle removal, detects and merges NEAR-DUPLICATE idle animations
        using content signatures (bone overlap + channel amplitude).
        """
        removed = []
        cfg = self.config

        # Step 1: Find all idle-like animations
        idle_like = {}
        for anim_name, anim_data in all_animations.items():
            name_lower = anim_name.lower()
            is_idle = (name_lower == 'idle' or
                       any(alias in name_lower for alias in cfg.idle_name_aliases) or
                       'idle' in name_lower)
        ...
        # Step 2: Find and remove static idles (keep the best one)
        ...
        # Step 3: Find near-duplicate idles by bone overlap
...
def batch_convert(input_dir: str, output_dir: str,
    ...
    """Batch convert all .bbmodel files in a directory tree (v9).
