#!/usr/bin/env python3
"""
BBModelAnimationConverter - Universal Animation Converter (v6)
==============================================================
...
Key Improvements over v5:
  - TRANSITION ZONE BLEND: Replaces destructive Symmetric Velocity Blend with
    a non-destructive transition zone approach. Only modifies the last 15-25%
    of the animation near the loop boundary, preserving the original shape
    for 75-85% of the animation. Uses C2-smooth quintic Hermite blending
    within the transition zone for tangent continuity without replacing
    the entire animation signal.
  - PERIODIC ANIMATION ENHANCER: Detects periodic motions (walk, fly, swim)
    by name pattern and autocorrelation analysis. Auto-extends incomplete
    cycles, mirrors partial cycles to complete them, and ensures at least
    one full period is present for smooth looping.
  - ENHANCED LOOP DETECTION: Zero-velocity crossing heuristic for walk
    cycles, tempo detection (0.4-1.2s common MC walk periods), symmetry
    checking for biped/quadruped gaits.
  - SMARTER EMPTY/DUPLICATE HANDLING: No empty .animation.json files written,
    bone-coverage-aware merging (won't merge if >30% bone coverage lost),
    tempo-aware dedup (animations with different speeds are kept separate).
  - IMPROVED QUALITY SCORING: Periodicity score, transition smoothness
    score, better distinction between "correctly static" and "broken static".
  - ENHANCED TEXTURE MAPPING VALIDATION: UV bounds checking, mirror_uv
    correction for all face types.

Inherited from v5:
  - GLOBAL CUBIC C1 CORRECTION (primary method for non-bounce cases)
  - HYBRID C1 ENFORCER (global cubic → transition zone blend → static snap)
  - PER-CHANNEL ADAPTIVE BLEND WINDOW
  - ENHANCED EMPTY ANIMATION CONSOLIDATION
  - ENHANCED DURATION OPTIMIZATION with harmonic search
...
class ConverterConfig:
    ...
    transition_zone_bounce_damp: float = 0.5  # velocity damping factor for bounce cases in zone

    # --- Periodic Animation Enhancement (v6 NEW) ---
    ...
    periodic_name_patterns: tuple = ('walk', 'run', 'fly', 'swim', 'crawl', 'gallop', 'trot', 'flap', 'bounce', 'hop', 'strafe')
...
class PeriodicAnimationEnhancer:
    ...
    def detect_periodicity(
        ...
        Returns:
            {
                ...
                'periodicity_score': float (0-1),
                'cycle_completeness': float (0-1),
                'name_match': bool,
                'detected_by': str,
                'should_enhance': bool,
            }
        ...
        # Collect all channels for period detection
        all_values = []
        for bone_name, channels in bone_channels.items():
            for channel, keyframes in channels.items():
                if len(keyframes) < 4:
                    continue
                # Resample at 60 Hz for period detection
                ...
                amplitude = max(abs(v) for v in centered)
                if amplitude > 0.01:  # Only use channels with significant motion
        ...
        if detected_period is None and name_match:
            # For name-matched animations, try common periods
            best_common = None
            best_strength = 0.0
            for T in self.COMMON_WALK_PERIODS:
                if T > duration:
                    continue
                # Check how well this period fits
            ...
            if best_common and best_strength > cfg.periodic_autocorrelation_threshold * 0.7:
                ...
                result['detected_by'] = 'common_period'

        # Step 4: Compute periodicity score
        if detected_period is not None:
            # Score based on autocorrelation strength and name match
            base_score = autocorr_strength
            if name_match:
                base_score = min(1.0, base_score * 1.3)  # boost for name match
    ...
    def enhance_periodic(
        ...
        Strategies:
        1. If duration < period and close to half-period: mirror to complete cycle
        2. If duration < period: extend by repeating with overlap blend
        3. If duration ≈ N * period: adjust duration to exact period multiple
        4. Set optimal loop point at detected period boundary

        Returns:
            (enhanced_bone_channels, new_duration, enhancement_info)
        """
        ...
        if abs(duration - target_duration) < period * 0.15:
            ...
            if abs(target_duration - duration) > 0.01:
                # Trim or extend keyframes to target duration
        ...
        # Strategy: If duration is roughly half-period, mirror to complete
            ...
            # Mirror the first half to create a complete cycle
        ...
        for bone_name, channels in bone_channels.items():
            ...
            for channel, keyframes in channels.items():
                ...
                if trimmed and abs(trimmed[-1][0] - target_duration) > 0.001:
                    # Interpolate to get value at target_duration
    ...
    def _extend_channels(
        ...
        for bone_name, channels in bone_channels.items():
            ...
            for channel, keyframes in channels.items():
                ...
                # Create extended data by repeating
                ...
                # Downsample back to reasonable keyframe count
                # Use DP simplification
    ...
    def _mirror_cycle(
        ...
        For walk animations, the second half is typically the mirror of the first.
        For rotation channels, we negate the mirrored values.
        For position channels, we keep the same values.
        """
            ...
            for channel, keyframes in channels.items():
                ...
                for t, v in resampled:
                    ...
                    if new_t > half_duration + 0.001:
                        # For rotation: negate to mirror the motion
                        # For position: keep same (symmetric stride)
                        if is_rotation:
                            new_v = -v  # Mirror rotation
                        else:
                            new_v = v   # Position is symmetric
                        extended.append((new_t, new_v))

                extended.sort(key=lambda x: x[0])

                # Ensure endpoints match for loop
                if extended:
                    p0 = extended[0][1]
                    pT = extended[-1][1]
                    # Snap last point to first for C0
                    if abs(pT - p0) > 0.001:
...
# ============================================================================
# C1 Continuity Enforcer (v6: Global Cubic Correction + Transition Zone Blend)
# ============================================================================
...
class C1ContinuityEnforcer:
    ...
    def _apply_transition_zone_blend(
        ...
        """v6: Apply transition zone blend for loop continuity.

        NON-DESTRUCTIVE: Only modifies the last N% of the animation
        (the "transition zone"), preserving the original shape for
        75-85% of the animation. Uses C2-smooth quintic Hermite blending
        within the transition zone.

        For bounce cases (v0*vT < 0), adds gentle velocity damping
        within the zone instead of replacing the entire animation.

        The transition zone blends from the original animation values
        at zone_start to (p0, v0) at t=duration, ensuring both C0
        position match and C1 velocity match at the loop boundary.
        ...
        # For bounce cases, use a slightly larger zone for smoother damping
        if is_bounce:
        ...
        if zone_start_idx < 1 or zone_start_idx >= n - 1:
            # Zone too small, fall back to simple snap
            if result:
        ...
        if is_bounce:
            # For bounce cases, add velocity damping within the zone
            # Instead of abruptly changing velocity, gradually dampen
            # the end velocity to match the start velocity
            damp = cfg.transition_zone_bounce_damp

            # Adjust zone start velocity to be closer to zero
            # (dampen the transition into the target velocity)
            v_zone_start = v_zone_start * (1.0 - damp * 0.3)

        # Apply quintic Hermite blend within the transition zone
        ...
        for i in range(zone_start_idx, n):
            ...
            # Quintic Hermite: (p_zone_start, v_zone_start, accel=0) -> (p_end_target, v_end_target, accel=0)
            new_val = self._quintic_hermite(
        ...
        for bone_name, channels in bone_channels.items():
            for channel, keyframes in channels.items():
                ...
                if correction_ratio <= cfg.global_cubic_distortion_limit:
                    ...
                    # ====================================================
                    # TRANSITION ZONE BLEND (v6: bounce case)
                    # ====================================================
                    # When v0*vT < 0, the global cubic creates S-shaped
                    # distortion. Instead of replacing the ENTIRE animation
                    # (v5 symmetric blend), only modify the last 20-35%
                    # transition zone with velocity damping.
                    # ====================================================
                    ...
                    # v6: Higher fidelity because only transition zone is modified
                    ...
                    blend_diag['bridge_details'].append({
                        ...
                        'method': 'transition_zone_blend',
                    })

                else:
                    # ====================================================
                    # TRANSITION ZONE BLEND (v6: non-bounce high distortion)
                    # ====================================================
                    # v6: Use transition zone blend instead of the old
                    # local blend. This is less destructive because it
                    # only modifies the transition zone, not the blend
                    # window which could be up to 40% of the animation.
                    # ====================================================
                    blend_diag['local_blend_count'] += 1
...
class QualityReporter:
    ...
    def report(
        ...
        # v6: Periodicity bonus — well-periodic animations loop better
        if report.periodicity_score > 0.7:
            score = min(100.0, score + 2.0)  # bonus for detected periodicity

        # v6: Transition smoothness bonus
        if report.transition_smoothness > 0.9:
            score = min(100.0, score + 2.0)  # bonus for smooth transition

        # v6: Preservation bonus — less modification = more faithful to original
        if report.original_animation_preserved > 0.95:
            score = min(100.0, score + 1.0)  # bonus for high preservation
...
class BBModelAnimationConverter:
    """Universal animation converter for .bbmodel files (v6).
    ...
    Pipeline:
      ...
      3. For periodic animations: detect and enhance periodicity (v6 NEW)
      4. For loop animations: detect optimal loop duration (with harmonic search)
      5. Enforce C1 continuity at loop boundaries (global cubic + transition zone blend)
      6. Simplify keyframes
      7. Build GeckoLib .animation.json
      8. Generate quality report with periodicity, transition smoothness metrics

    v6 Improvements:
      - Transition Zone Blend: non-destructive loop continuity (preserves 75-85% of animation)
      - Periodic Animation Enhancer: auto-detect and complete walk/fly/swim cycles
      - Smarter empty/duplicate handling: no empty files, bone-coverage-aware merging
      - Improved quality scoring: periodicity score, transition smoothness, preservation ratio
    ...
    def __init__(self, config: ConverterConfig = None):
        ...
        self.periodic_enhancer = PeriodicAnimationEnhancer(self.config)  # v6 NEW
    ...
    def convert_file(self, bbmodel_path: str,
        ...
        for anim_name, anim_data in extracted['animations'].items():
            ...
            # Step 3.5: v6 Periodic Animation Enhancement
            periodicity_info = {
                'is_periodic': False,
                'period': None,
                'periodicity_score': 0.0,
                ...
                if periodicity_info.get('should_enhance', False):
                    ...
                    bone_channels, current_duration, enhance_info = \
                        self.periodic_enhancer.enhance_periodic(
                            bone_channels, current_duration, periodicity_info, interpolation
                        )
            ...
            # v6: Periodicity and transition smoothness metrics
            ...
            # v6: Compute transition smoothness score
            ...
            # v6: Compute original animation preservation ratio
            if periodic_enhanced:
                qreport.original_animation_preserved = 0.7  # approximate
            elif blend_diag.get('global_cubic_count', 0) > 0:
                qreport.original_animation_preserved = 0.95  # global cubic preserves most
            elif blend_diag.get('local_blend_count', 0) > 0:
                qreport.original_animation_preserved = 1.0 - self.config.transition_zone_ratio
            else:
                qreport.original_animation_preserved = 1.0  # no changes
        ...
        if output_path:
            # v6: Don't write empty .animation.json files
            if all_animations and not self.config.skip_empty_animation_files:
