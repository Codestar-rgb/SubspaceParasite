#!/usr/bin/env python3
"""
BBModelAnimationConverter - Universal Animation Converter (v19)
==============================================================
...
Key Improvements over v18 (5 major fixes):
...
class ConverterConfig:
    """Master configuration for BBModelAnimationConverter v19."""
    # --- Loop Detection ---
...
class BBModelAnimationConverter:
    ...
    def convert_file(self, bbmodel_path: str,
        ...
        for anim_name, anim_data in extracted['animations'].items():
                        ...
                        if (optimal_duration > current_duration and is_walk_anim and
                            ...
                            if abs(ratio - round(ratio)) < 0.15 and round(ratio) >= 2:
                                ...
                                bone_channels = self._replicate_walk_keyframes(
                                    ...
                                    use_periodic_extrapolation=getattr(cfg, 'walk_periodic_extrapolation', True)
                                )
            ...
            # v19 NEW: Walk cycle extension for too-fast walks
            # Humanoid walks < 0.8s are unrealistically fast; extend by repeating
            is_walk_anim = any(p in anim_name.lower() for p in ('walk', 'run'))
            if (is_walk_anim and current_duration < 0.8 and loop_mode == "loop"
                ...
                if c0_ok:
                    ...
                    if new_duration > current_duration:
                        ...
                        duration_change_reason = (
                            f"v19 walk extended: {current_duration:.4f}s -> {new_duration:.4f}s "
                            f"(factor={extension_factor}, too-fast humanoid walk)"
                        )
    ...
    def _synthesize_walk_missing_bones(
        ...
        """v19: Synthesize subtle motion for bones present in other animations
        ...
        for bone_name in sorted(missing_bones):
            ...
            if is_head:
                ...
                amplitude = min(idle_amplitude * 0.3, 3.0) if idle_amplitude > 0.5 else 1.5
                rx_kfs = []
                ...
                amplitude_z = min(idle_amplitude * 0.15, 1.5) if idle_amplitude > 0.5 else 0.5
                rz_kfs = []
                for i in range(n_kfs + 1):
                    t = round(i * walk_duration / n_kfs, 4)
                ...
                amplitude = min(idle_amplitude * 0.2, 1.0) if idle_amplitude > 0.5 else 0.3
                rx_kfs = []
                for i in range(n_kfs + 1):
                    t = round(i * walk_duration / n_kfs, 4)
                ...
                amplitude = min(idle_amplitude * 0.25, 2.0) if idle_amplitude > 0.5 else 0.8
                rx_kfs = []
                for i in range(n_kfs + 1):
                    t = round(i * walk_duration / n_kfs, 4)
                ...
                amplitude = min(idle_amplitude * 0.2, 1.5) if idle_amplitude > 0.5 else 0.5
                rz_kfs = []
                for i in range(n_kfs + 1):
                    t = round(i * walk_duration / n_kfs, 4)
    ...
    def _replicate_walk_keyframes(
        ...
        """v18 NEW: Replicate walk keyframes for duration extension.
        ...
        For periodic extrapolation, the last keyframe of each cycle is adjusted
        to smoothly connect to the first keyframe of the next cycle, ensuring
        C0 continuity between replicated cycles.
        ...
        for bone_name, channels in bone_channels.items():
            ...
            for channel, keyframes in channels.items():
                ...
                # Check if the last keyframe is exactly at cycle_duration
                last_t = sorted_kfs[-1][0]
                first_val = sorted_kfs[0][1]

                # Build replicated keyframes
                all_kfs = []

                for cycle in range(n_cycles):
                    time_offset = cycle * cycle_duration

                    # For each cycle, add keyframes with time offset
                    # Skip the last keyframe of each cycle (except the final one)
                    # because it's the same as the first keyframe of the next cycle
                ...
                # Ensure the last keyframe matches the first keyframe value (C0)
                if all_kfs:
                    total_duration = n_cycles * cycle_duration
                    # Replace the last keyframe's value with the first keyframe's value
                    # This guarantees C0 continuity at the loop boundary
                    last_kf = all_kfs[-1]
                ...
                if use_periodic_extrapolation and len(all_kfs) > 6:
                    # Smooth the transition between cycles using cosine blending
                    # This prevents sharp transitions at cycle boundaries
    ...
    def _walk_lightweight_c1_enforcement(
        ...
        """v18 NEW: Lightweight C1 enforcement specifically for walk animations.

        Walk animations have naturally periodic structure (sinusoidal leg motion).
        The standard C1 enforcement (full_resample with transition zones) destroys
        this structure by introducing oscillations. This method instead:

        1. Evaluates the current C0/C1 state
        2. If C0 is perfect (within 0.1 deg), only applies a minimal
           velocity bridge at the loop boundary over 3-5% of duration
        3. If C0 is not perfect, applies a gentle cosine bridge that
           both fixes C0 and smooths C1
        4. Preserves the original keyframe structure as much as possible
        ...
        blend_diag = {
            ...
            'quintic_correction_count': 0,
            'quintic_hermite_zone_count': 0,
            'walk_lightweight_c1': True,  # v18 marker
        }

        # Step 1: First try standard C1 enforcement (it's well-tested)
        # but limit the transition zone for walks
        original_tz_ratio = cfg.transition_zone_ratio
        original_tz_max = cfg.transition_zone_max_ratio

        # Temporarily reduce transition zone for walks
        walk_tz_ratio = getattr(cfg, 'walk_c1_transition_zone_ratio', 0.05)
        cfg.transition_zone_ratio = walk_tz_ratio
        cfg.transition_zone_max_ratio = walk_tz_ratio * 1.5

        try:
            # Use the standard enforcer with reduced transition zone
            if hasattr(self.c1_enforcer, 'enforce_with_full_resample'):
                bone_channels, blend_diag = self.c1_enforcer.enforce_with_full_resample(
                    bone_channels, duration, interpolation,
                    anim_name=anim_name,
                    cached_resampled=cached_resampled,
                    periodicity_info=None,
                    is_sleeping=False,
                )
            elif hasattr(self.c1_enforcer, 'enforce'):
                bone_channels, blend_diag = self.c1_enforcer.enforce(
                    bone_channels, duration, interpolation,
                    cached_resampled=cached_resampled,
                    periodicity_info=None,
            ...
            # Restore original transition zone settings
            cfg.transition_zone_ratio = original_tz_ratio
            cfg.transition_zone_max_ratio = original_tz_max

        # Step 2: Apply cosine bridge for walk loop boundary smoothness
        if getattr(cfg, 'walk_c1_cosine_bridge', True):
            bone_channels = self._apply_walk_cosine_bridge(
                bone_channels, duration, interpolation
            )
            blend_diag['walk_cosine_bridge_applied'] = True

        # Step 3: Verify C0 and snap if needed
        for bone_name, channels in bone_channels.items():
            for channel, keyframes in channels.items():
                if len(keyframes) < 2:
                    continue
    ...
    def _apply_walk_cosine_bridge(
        self,
        bone_channels: Dict[str, Dict[str, List[Tuple[float, float]]]],
        duration: float,
        interpolation: str,
        bridge_ratio: float = 0.05
    ) -> Dict[str, Dict[str, List[Tuple[float, float]]]]:
        """v18 NEW: Apply cosine bridge at walk loop boundary.

        For each channel, compute the velocity at the start and end of the
        animation. If they don't match, apply a smooth cosine correction
        over the last `bridge_ratio` fraction of the animation to make
        the end velocity match the start velocity.

        This is much gentler than the standard C1 enforcement and preserves
        the natural sinusoidal structure of walk animations.
        """
        bridge_duration = duration * bridge_ratio
        ...
        for bone_name, channels in bone_channels.items():
            ...
            for channel, keyframes in channels.items():
                ...
                # Compute velocities at start and end
                n = len(sorted_kfs)
                dt_start = sorted_kfs[1][0] - sorted_kfs[0][0] if n > 1 else 0.01
                dt_end = sorted_kfs[-1][0] - sorted_kfs[-2][0] if n > 1 else 0.01
                ...
                c1_error = abs(v_start - v_end)
                if c1_error < 0.5:
                    # C1 is already good enough
                    result[bone_name][channel] = sorted_kfs
                    continue

                # Apply cosine bridge correction to keyframes in the bridge zone
                bridge_start_t = duration - bridge_duration
                new_kfs = []

                for t, v in sorted_kfs:
                    if t >= bridge_start_t:
                        # This keyframe is in the bridge zone
                        # Compute the correction
                        alpha = (t - bridge_start_t) / bridge_duration
                        alpha = max(0.0, min(1.0, alpha))

                        # Cosine weight: 0 at bridge_start, 1 at duration
                        weight = 0.5 * (1.0 - math.cos(math.pi * alpha))

                        # Correction: smoothly transition from current velocity
                        # to target velocity
                        # The target is to have v_end match v_start at the loop boundary
                        velocity_diff = v_end - v_start
                        correction = -velocity_diff * weight * (t - bridge_start_t) * 0.5

                        # Scale correction by distance from bridge start
                        max_correction = abs(velocity_diff) * bridge_duration * 0.25
                        if abs(correction) > max_correction:
                            correction = math.copysign(max_correction, correction)

                        new_v = v + correction

                        # Ensure C0 is maintained at the boundary
                        if abs(t - duration) < 0.001:
                            new_v = sorted_kfs[0][1]  # Exact C0 match

                        new_kfs.append((t, new_v))
                    else:
                        new_kfs.append((t, v))

                result[bone_name][channel] = new_kfs
    ...
    def _exact_duplicate_consolidation(
        ...
        """v18 NEW: Consolidate animations with identical output data.
        ...
        Priority order for keeping: idle > walk > attack > death > evolved > other
        This ensures that for ferCow-style models where idle/evolved/attack are
        identical, we keep "idle" as the canonical name.
        ...
        # v19 FIX: Protected-category awareness — do NOT consolidate across
        # different protected categories. Attack/evolved/idle are distinct
        # game states even if data is identical.
        PROTECTED_CATEGORIES = {'attack', 'walk', 'idle', 'sleep', 'death', 'evolved'}
        ...
        for h, group in hash_groups.items():
            ...
            # v19: Group by protected category — only consolidate within same category
            category_groups = {}  # category -> [(anim_name, priority)]
            for anim_name, pri in group:
                cat = _get_category(anim_name)
                if cat not in category_groups:
                    category_groups[cat] = []
                category_groups[cat].append((anim_name, pri))

            # For each category group, sort by priority and keep the best one
            for cat, cat_group in category_groups.items():
                if len(cat_group) <= 1:
                    continue
                cat_group.sort(key=lambda x: x[1])
                kept_name = cat_group[0][0]
                kept_category_str = cat
                for dup_name, _ in cat_group[1:]:
                ...
                if len(cat_group) > 1:
                    consolidated_names = [name for name, _ in cat_group[1:]]
                    if kept_name in all_animations:
                        all_animations[kept_name]['_v19_consolidated_from'] = consolidated_names
                        all_animations[kept_name]['_v19_consolidation_note'] = (
                            f"Consolidated same-category duplicates: {', '.join(consolidated_names)} "
                            f"(kept as {kept_category_str} by priority)"
                        )
...
def batch_convert(input_dir: str, output_dir: str,
    ...
    """
    print("=" * 70)
    print("  Universal BBModel Animation Converter (v19)")
    print("  .bbmodel -> .animation.json with C1+C2 Correction & v19 Improvements")
    ...
    print()
    print("=" * 70)
    print("  DONE - Universal BBModel Animation Converter (v19)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Universal BBModel Animation Converter with C1+C2 Correction & v19 Improvements"
    )
