#!/usr/bin/env python3
"""
BBModelAnimationConverter - Universal Animation Converter (v11)
==============================================================
...
Key Improvements over v10 (7 major improvements):
...
class ConverterConfig:
    ...
    global_cubic_distortion_limit: float = 0.50  # max correction/amplitude ratio before fallback (v10: raised from 0.30)
    ...
    c2_distortion_limit: float = 0.55              # max correction/amplitude before falling back to cubic (v10: raised from 0.40)
    ...
    quintic_distortion_limit: float = 0.55         # max correction/amplitude before falling back to cubic (v10: raised from 0.35)
...
class BBModelAnimationConverter:
    ...
    def convert_file(self, bbmodel_path: str,
        ...
        for anim_name, anim_data in extracted['animations'].items():
            ...
            # Step 11c [v11 NEW]: FINAL C0 Enforcement — GUARANTEED 100% C0 Continuity
            ...
            # Step 11d [v11 NEW]: Walk Validation — ensure leg bones have sufficient keyframe coverage
            if (loop_mode == "loop" and not is_static and
                (walk_full_cycle_reconstructed or walk_half_cycle_mirrored) and
                ...
                if walk_kf_generated > 0:
                    ...
                    # Re-build after walk validation
                    anim_json = self.json_builder.build(
                        anim_name, loop_mode, bone_channels, current_duration
                    )
                    all_animations[anim_name] = anim_json
        ...
        if output_path:
            ...
            if should_write and hasattr(cfg, 'skip_files_with_only_static') and cfg.skip_files_with_only_static:
                only_static = True
                for anim_name, anim_data in all_animations.items():
                    if not anim_data.get('static', False):
                        only_static = False
    ...
    def _walk_validation_resample(
        ...
        """v11 Improvement 2: Walk Validation — ensure leg bones have sufficient keyframes.

        After walk cycle reconstruction, checks if leg bone channels have at least
        walk_min_keyframes_per_channel (8) keyframes. If not, generates additional
        keyframes by resampling the reconstructed curve at regular intervals.
        ...
        for bone_name, channels in bone_channels.items():
            ...
            for channel, keyframes in channels.items():
                if not is_leg or len(keyframes) >= min_kfs:
                    result_channels[bone_name][channel] = keyframes
                    continue
...
def batch_convert(input_dir: str, output_dir: str,
    ...
    """Batch convert all .bbmodel files in a directory tree (v11).

    For each .bbmodel file:
      - Extract geo.json + texture (using bbmodel_to_geo.py)
      - Extract and convert animations (using this v11 converter)
    ...
    """
    print("=" * 70)
    print("  Universal BBModel Animation Converter (v11)")
    print("  .bbmodel -> .animation.json with C1+C2 Correction & v11 Improvements")
    ...
    print()
    print("=" * 70)
    print("  DONE - Universal BBModel Animation Converter (v11)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Universal BBModel Animation Converter with C1+C2 Correction & v11 Improvements"
    )
